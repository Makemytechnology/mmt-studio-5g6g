...
class GnbLibrary:
    ...
    @keyword("Provision PLMN On Core")
    def provision_plmn(self, mcc, mnc, name="test-plmn"):
        """POST the PLMN into the core's served list (live — the AMF
        rebuilds its NG Setup PLMN view from the DB on this call)."""
        from src.core.api import core_api
        resp = core_api("/api/plmn/supported", method="POST",
                        body={"mcc": str(mcc), "mnc": str(mnc), "name": name,
                              "plmn_type": "equivalent", "priority": 10})
        if not resp or not resp.get("ok"):
            raise AssertionError(f"PLMN provisioning failed: {resp}")

    @keyword("Remove PLMN From Core")
    def remove_plmn(self, mcc, mnc):
        from src.core.api import core_api
        core_api(f"/api/plmn/supported/{mcc}-{mnc}", method="DELETE", quiet=True)
    ...
    def create_gnb_from_config(self, config_name, **overrides):
        ...
        for s in slices:
            if isinstance(s.get("sd"), str) and s["sd"].startswith("0x"):
                s["sd"] = int(s["sd"], 16)
        # Unique name + Global RAN Node ID per instance: creating the
        # same profile twice (multi-gNB tests, TS 23.501 §5.2.1) must
        # not overwrite the prior machine in the registry or reuse its
        # gNB ID toward the AMF.
        gnb_name = profile.get("gnb_name", config_name)
        base_name, n = gnb_name, 1
        while gnb_name in self._gnbs:
            n += 1
            gnb_name = f"{base_name}-{n}"
            gnb_id += 1
        gnb = GnbStateMachine(

...
TC-NGS-004 NG Setup With Custom PLMN
    ...
    # The AMF only accepts NG Setup for PLMNs in its served list
    # (TS 38.413 §8.7.1) — provision 310/260 first.
    Provision PLMN On Core    310    260
    ${gnb}=    Create gNB From Config    ${GNB_NAME}    mcc=310    mnc=260    tac=0100
    Connect gNB And Wait Ready    ${gnb}    timeout=${SETUP_TIMEOUT}
    gNB Should Be Ready    ${gnb}
    Log    TC-NGS-004 PASS: NG Setup with PLMN 310/260 TAC 0100
    [Teardown]    Run Keywords    Remove gNB    ${gnb}
    ...    AND    Remove PLMN From Core    310    260
...
TC-NGS-015 NG Setup Timing Measurement
    ...
    ${elapsed_ms}=    Evaluate    round(${elapsed_ms}, 1)
    Log    NG Setup latency: ${elapsed_ms} ms
    gNB Should Be Ready    ${gnb}
    Should Be True    ${elapsed_ms} < 5000
    ...    NG Setup should complete within 5 seconds (took ${elapsed_ms}ms)
    Log    TC-NGS-015 PASS: NG Setup latency=${elapsed_ms}ms
    [Teardown]    Remove gNB    ${gnb}

...
from src.statemachine.dstt import DSTT
...
class UeStateMachine:
    ...
    def __init__(self, sim, gnb=None):
        ...
        self.pdu_sessions = {}
        # Rel-19 TSN: DS-TT translators per Ethernet PDU session
        # (psi → DSTT). Empty for plain IP sessions.
        self.dstts = {}
        self._pti_counter = 1
    ...
    def establish_pdu_session(self, dnn="internet", sst=1, sd=None, pdu_session_id=1,
                              pdu_session_type=1, dstt_mac=None, residence_time_ns=None):
        """Establish PDU session — TS 24.501 §8.3.1.

        Rel-19 TSN: pass pdu_session_type=5 (Ethernet) plus dstt_mac /
        residence_time_ns to create a DS-TT for the session (TS 23.501
        §5.28.1). A DSTT object is registered under self.dstts[psi] and
        its initial PMIC (a capability report) rides the establishment
        request so the TSN AF learns the port immediately.
        """
        ...
        pmic = None
        if pdu_session_type == 5:  # Ethernet — create the DS-TT
            if dstt_mac is None:
                # Derive a stable locally-administered MAC from the IMSI.
                tail = int(self.imsi[-8:]) & 0xFFFFFFFF
                dstt_mac = bytes([0x02, 0x00,
                                  (tail >> 24) & 0xFF, (tail >> 16) & 0xFF,
                                  (tail >> 8) & 0xFF, tail & 0xFF])
            if residence_time_ns is None:
                residence_time_ns = 100_000  # 100 µs default
            dstt = DSTT(dstt_mac, residence_time_ns)
            self.dstts[pdu_session_id] = dstt
            pmic = dstt.capability_report()

        plain = NasBuilder.ul_nas_transport_pdu_session(
            pdu_session_id, pti, dnn, sst, sd,
            pdu_session_type=pdu_session_type,
            dstt_mac=dstt_mac, residence_time_ns=residence_time_ns, pmic=pmic)
        self._send_secured(plain)
        self._log("PDU Session Request: PSI=%d DNN=%s type=%d%s", pdu_session_id, dnn,
                  pdu_session_type,
                  f" DS-TT={self.dstts[pdu_session_id].mac_str()}" if pdu_session_type == 5 else "")
        return True
    ...
    def _process_gsm(self, msg_type, psi, inner):
        ...
        if msg_type in (0xC1, 0xC2):  # PDU Session Establishment Accept
            ...
            if pco:
                session.update(pco)
            # Rel-19 TSN: Ethernet PDU sessions carry no UE IP — the
            # DS-TT MAC identifies the port instead (TS 23.501 §5.28.1).
            if psi in self.dstts:
                session["pdu_type"] = "ethernet"
                session["dstt_mac"] = self.dstts[psi].mac_str()
            self.pdu_sessions[psi] = session
            self._log("PDU Session %d: IP=%s QoS=%d rules%s", psi, ip, len(qos_info),
                      f" DS-TT={session['dstt_mac']}" if psi in self.dstts
                      else (f" P-CSCF={pco['pcscf']}" if pco.get('pcscf') else ""))
            ...
            # Rel-19 TSN: a Port management information container (0x74)
            # carries a TS 24.539 PMIC the TSN AF / TSCTSF sent to the
            # DS-TT (gate schedule, PTP instance, …). Terminate it and
            # return the DS-TT's reply PMIC in the Modification Complete
            # (TS 23.501 §5.28.3.2 DS-TT→AF path).
            reply_pmic = self._handle_dstt_pmic(psi, inner)
            self._send_modification_complete(psi, pmic=reply_pmic)
    ...
    def _process_gsm_decoded(self, msg_type, psi, ip="unknown", pco=None, raw=None):
        ...
        if msg_type in (0xC1, 0xC2, 193, 194):  # PDU Session Establishment Accept
            ...
            if pco:
                session.update(pco)
            # Rel-19 TSN: mark Ethernet sessions here too — this decode
            # path must mirror _process_gsm (TS 23.501 §5.28.1, no UE IP).
            if psi in self.dstts:
                session["pdu_type"] = "ethernet"
                session["dstt_mac"] = self.dstts[psi].mac_str()
            self.pdu_sessions[psi] = session
            ...
            reply_pmic = self._handle_dstt_pmic_from_list(psi, raw)
            self._send_modification_complete(psi, pmic=reply_pmic)
    ...
    def _send_modification_complete(self, psi, pmic=None):
        """Send PDU Session Modification Complete (NAS type 0xCC/204) via secured UL NAS.

        TS 24.501 §8.3.10 — UE acknowledges dedicated bearer activation.
        When the DS-TT produced a reply PMIC (Rel-19 TSN), it rides the
        Port management information container IE (0x74, §9.11.4.27) —
        a TLV-E: IEI + 2-octet length + value.
        """
        # Minimal 5GSM header: EPD=0x2E, PSI, PTI=0, Type=0xCC.
        gsm_bytes = bytes([0x2E, psi, 0x00, 0xCC])
        if pmic:
            gsm_bytes += bytes([0x74]) + len(pmic).to_bytes(2, 'big') + bytes(pmic)
        plain = NasBuilder.ul_nas_transport_raw(psi, gsm_bytes)
        self._send_secured(plain)
        self._log("TX PDU Session Modification Complete (PSI=%d%s)", psi,
                  f", PMIC {len(pmic)}B" if pmic else "")

    def _handle_dstt_pmic(self, psi, inner):
        """Extract the Port management information container (0x74) from a
        pycrate-decoded Modification Command and terminate it at the
        session's DS-TT. Returns the reply PMIC bytes or None."""
        dstt = self.dstts.get(psi)
        if dstt is None:
            return None
        pmic = None
        try:
            elt = inner['PortMgmtInfoContainer']['V']
            # pycrate may hold raw bytes OR a parsed TS 24.539 container
            # object (parse_PortMgmtInfoCont hook) — to_bytes() re-encodes
            # either back to the wire form.
            try:
                pmic = elt.to_bytes()
            except Exception:
                pmic = bytes(elt.get_val())
        except Exception:
            return None
        if not pmic:
            return None
        return self._dstt_terminate(dstt, psi, pmic)

    def _handle_dstt_pmic_from_list(self, psi, raw):
        """Same, for the pycrate pre-decoded list form."""
        dstt = self.dstts.get(psi)
        if dstt is None or not raw:
            return None
        for ie in raw[1:]:
            if isinstance(ie, list) and len(ie) >= 2 and ie[0] in (0x74, 116):
                val = ie[2] if len(ie) > 2 else ie[1]
                pmic = None
                if isinstance(val, (bytes, bytearray)):
                    pmic = bytes(val)
                elif isinstance(val, list) and val and isinstance(val[0], int):
                    # pycrate's parse_PortMgmtInfoCont hook decoded the
                    # TS 24.539 container into a structured value —
                    # re-encode it back to the wire form for the DS-TT.
                    try:
                        from pycrate_mobile.NAS5G import FGTSNAFEthPortTypeClasses
                        cont = FGTSNAFEthPortTypeClasses[val[0]]()
                        cont.set_val(val)
                        pmic = cont.to_bytes()
                    except Exception as e:
                        self._log("PMIC re-encode failed: %s", e)
                if pmic:
                    return self._dstt_terminate(dstt, psi, pmic)
        return None

    def _dstt_terminate(self, dstt, psi, pmic):
        try:
            reply = dstt.handle_pmic(pmic)
        except Exception as e:
            self._log("DS-TT PSI=%d PMIC parse error: %s", psi, e)
            return None
        self._log("DS-TT PSI=%d: PMIC %dB terminated → reply %s (gate=%s)",
                  psi, len(pmic), f"{len(reply)}B" if reply else "none", dstt.gate_enabled)
        return reply

...
except Exception:  # pragma: no cover - pysctp not installed (codec-only envs)
    SctpClient = None
...
class GnbStateMachine:
    ...
    def __init__(self, amf_ip, amf_port, gnb_id=None, gnb_name=None,
        ...
        # Rel-19 TSN: TSC Assistance Information per PDU session, keyed
        # psi -> {qfi -> {"ul"/"dl": {periodicity, burst_arrival_time_ns}}}.
        self.tscai_by_psi = {}
    ...
    def _handle_pdu_session_setup(self, ies):
        ...
        for item in ies.get(74, []):
            ...
            # Rel-19 TSN: capture the TSC Assistance Information the SMF
            # attached per QoS flow (TS 38.413 TSCTrafficCharacteristics,
            # TS 23.501 §5.27.2.4). A real gNB uses it to schedule the
            # radio grant; the tester records it so Robot suites can
            # assert the periodicity / burst arrival time reached the RAN.
            if isinstance(transfer_bytes, (bytes, bytearray)):
                try:
                    tscai = NgapCodec.decode_tscai(transfer_bytes)
                    if tscai:
                        self.tscai_by_psi[psi] = tscai
                        self._log("TSCAI received PSI=%d: %s (TS 23.501 §5.27.2.4)", psi, tscai)
                except Exception as e:
                    self._log("TSCAI decode PSI=%d: %s", psi, e)
    ...
    def _handle_pdu_session_modify(self, ies):
        ...
        for item in modify_list:
            ...
            # Rel-19 TSC: the N2 leg of a network-initiated modification
            # carries TSCTrafficCharacteristics on the QosFlowAddOrModify
            # list (TS 38.413 §9.3.4.9 / TS 23.502 §4.3.3 step 3). Decode
            # and record so suites can assert the TSCAI reached the RAN.
            xfer = item.get('pDUSessionResourceModifyRequestTransfer')
            tscai = {}
            if isinstance(xfer, (bytes, bytearray)) and len(xfer) > 0:
                tscai = NgapCodec.decode_tscai_modify(bytes(xfer))
            else:
                # pycrate auto-decodes the contained transfer to
                # ('PDUSessionResourceModifyRequestTransfer', {...}).
                val = xfer
                if isinstance(val, tuple) and len(val) == 2:
                    val = val[1]
                if isinstance(val, dict):
                    tscai = NgapCodec._extract_tscai(val, list_ie_id=135)
            if tscai:
                merged = dict(self.tscai_by_psi.get(psi) or {})
                merged.update(tscai)
                self.tscai_by_psi[psi] = merged
                self._log("TSCAI (modify) PSI=%d: %s", psi, tscai)
                # Rel-19 EnTSCAC (TS 23.501 §5.27.2.5): a real gNB
                # schedules the deterministic grant and reports the
                # achievable Burst Arrival Time offset back. The bench
                # emits a fixed +250µs DL offset via PDU Session
                # Resource Notify so suites can assert the feedback
                # reaches the TSN AF stream status.
                for fqfi, dirs in tscai.items():
                    try:
                        ntf = NgapCodec.build_pdu_session_resource_notify_tsc_feedback(
                            amf_id, ran_id, psi, fqfi,
                            bat_offset_us_dl=250 if 'dl' in dirs else None,
                            bat_offset_us_ul=250 if 'ul' in dirs else None)
                        self._trace_tx(ntf, "PDUSessionResourceNotify(TSC-feedback)", 30)
                        self._sctp.send(ntf, stream=self._stream_for_ue(ran_id))
                        self._log("TSC feedback sent PSI=%d QFI=%d (+250us)", psi, fqfi)
                    except Exception as e:
                        self._log("TSC feedback send failed: %s", e)
    ...
    def _on_pdu_session_accepted(self, ue, psi, ue_ip):
        ...
        # Rel-19 TSN: an Ethernet PDU session (type 5) carries no UE IP
        # and needs no local TUN — the DS-TT is simulated in-process and
        # the bench exercises the control plane, not L2 frame injection.
        # Answer success straight away with the allocated gNB DL TEID so
        # the SMF can activate the user plane and the 5GSM FSM reaches
        # ACTIVE (TS 23.502 §4.3.2.2.1 step 12) — otherwise later TSC
        # modifications are skipped ("session in state PENDING").
        if (ue.pdu_sessions.get(psi, {}) or {}).get('pdu_type') == 'ethernet':
            self._log("Ethernet PDU session PSI=%d — NGAP success without TUN "
                      "(no UE IP per TS 23.501 §5.28.1)", psi)
            _send_success()
            return

        # Pre-flight: missing UPF info means we can't build a tunnel at
        # all. Guard _gtpu — it is None when the bench runs without
        # CAP_NET_ADMIN; send the failure inline in that case instead of
        # crashing the UE FSM thread with an AttributeError (which
        # swallowed the NGAP response entirely).
        if not (upf_ip and remote_teid):
            fail = lambda: _send_failure(
                "transport-resource-unavailable",
                "UPF tunnel info missing from PSR Setup Request")
            self._gtpu.submit_setup(fail) if self._gtpu else fail()
            return
    ...
    def _handle_ue_context_release(self, ies):
        ...
        if ue:
            # TS 38.413 §8.3.3 releases ONE UE-associated logical NG
            # connection, identified by the UE NGAP ID pair (§9.3.3.2).
            # Under rapid dereg→re-register cycles the release for the
            # OLD connection can land after the UE already opened a NEW
            # one (fresh InitialUEMessage, new RAN UE NGAP ID). In that
            # case only the old mapping may be dropped — stomping the
            # UE object's state / current IDs here silently killed the
            # in-flight authentication (UE stuck AUTH_PENDING, AMF
            # T3560 retransmits into the void).
            stale = ue.ran_ue_ngap_id is not None and ue.ran_ue_ngap_id != ran_id
            if stale:
                self._log("UEContextRelease for superseded connection "
                          "ranUeID=%s (UE now on %s) — dropping old mapping only",
                          ran_id, ue.ran_ue_ngap_id)

...
# SctpClient pulls in the native `pysctp` extension (kernel SCTP). Keep
# it optional so pure-codec importers (NAS / NGAP / GTP-U / TS 24.539
# TSN codecs) work in lightweight / CI-lite environments without the
# native lib. The transport is imported eagerly by app.py / the state
# machines, which run only where pysctp is installed.
try:
    from src.protocol.sctp import SctpClient
except Exception:  # pragma: no cover - pysctp not installed
    SctpClient = None

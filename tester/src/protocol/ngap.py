...
class NgapCodec:
    ...
    @staticmethod
    def decode_tscai(transfer_bytes):
        """Rel-19: decode the TSC Assistance Information the SMF attaches
        per QoS flow in a PDUSessionResourceSetupRequestTransfer
        (TS 38.413 QosFlowSetupRequestList IE 136 → QosFlowSetupRequestItem
        protocol extension id 196 TSCTrafficCharacteristics).

        Returns {qfi: {"ul": {periodicity, burst_arrival_time_ns},
                       "dl": {...}}} for every flow that carries TSCAI.
        Empty dict when no TSC flow is present (plain sessions).
        """
        import logging
        _ngap_log = logging.getLogger("tester.ngap")
        try:
            ...
            _ngap_log.debug("decode_tscai: APER decode failed: %s", e)
            return {}

        return NgapCodec._extract_tscai(val, list_ie_id=136)

    @staticmethod
    def decode_tscai_modify(transfer_bytes):
        """Rel-19: decode TSCAI from a PDUSessionResourceModifyRequest-
        Transfer — the N2 leg of a network-initiated modification adding
        TSC flows after establishment (TS 38.413 §9.3.4.9,
        QosFlowAddOrModifyRequestList IE 135 → item extension id 196).
        Same return shape as decode_tscai()."""
        import logging
        _ngap_log = logging.getLogger("tester.ngap")
        try:
            with _lock:
                xfer = NGAP.NGAP_IEs.PDUSessionResourceModifyRequestTransfer
                xfer.from_aper(transfer_bytes)
                val = xfer.get_val()
        except Exception as e:
            _ngap_log.debug("decode_tscai_modify: APER decode failed: %s", e)
            return {}
        return NgapCodec._extract_tscai(val, list_ie_id=135)

    @staticmethod
    def _extract_tscai(val, list_ie_id):
        """Walk a decoded transfer's QoS-flow list IE (136 setup /
        135 add-or-modify) and collect the id-196 TSC extensions."""
        out = {}
        ies = val.get('protocolIEs', []) if isinstance(val, dict) else []
        for ie in ies:
            if not isinstance(ie, dict) or ie.get('id') != list_ie_id:
                continue
            flow_list = ie.get('value')
            if isinstance(flow_list, tuple) and len(flow_list) == 2:
            ...
            for item in flow_list:
                if not isinstance(item, dict):
                    continue
                qfi = item.get('qosFlowIdentifier')
                exts = item.get('iE-Extensions') or item.get('iEExtensions') or []
                for ext in exts:
                    if not isinstance(ext, dict) or ext.get('id') != 196:
                        continue
                    # pycrate decodes the open-type extension under
                    # 'extensionValue' as (TypeName, value); older
                    # encoders may use 'value'.
                    tc = ext.get('extensionValue', ext.get('value'))
                    if isinstance(tc, tuple) and len(tc) == 2:
                        tc = tc[1]
                    parsed = NgapCodec._parse_tsc_traffic_char(tc)
                    if parsed:
                        out[qfi] = parsed
        return out

    @staticmethod
    def _parse_tsc_traffic_char(tc):
        """TSCTrafficCharacteristics → {"ul"/"dl": {periodicity, burst_arrival_time_ns}}."""
        if not isinstance(tc, dict):
            return None
        res = {}
        for key, dir_name in (('tSCAssistanceInformationDL', 'dl'),
                              ('tSCAssistanceInformationUL', 'ul')):
            ai = tc.get(key)
            if not isinstance(ai, dict):
                continue
            entry = {}
            if 'periodicity' in ai:
                entry['periodicity'] = ai['periodicity']
            bat = ai.get('burstArrivalTime')
            if isinstance(bat, (bytes, bytearray)) and len(bat) >= 1:
                entry['burst_arrival_time_ns'] = int.from_bytes(bytes(bat), 'big')
            res[dir_name] = entry
        return res or None
    ...
    @staticmethod
    def build_pdu_session_resource_notify_tsc_feedback(amf_ue_ngap_id, ran_ue_ngap_id,
                                                       pdu_session_id, qfi,
                                                       bat_offset_us_dl=None,
                                                       bat_offset_us_ul=None,
                                                       adjusted_periodicity_us=None):
        """PDUSessionResourceNotify (TS 38.413 §8.2.4) carrying the Rel-19
        TSCTrafficCharacteristicsFeedback extension (id 393) on the QoS
        flow notify item — the RAN's Burst Arrival Time offset feedback
        for a TSC flow (TS 23.501 §5.27.2.5 EnTSCAC). Offsets in µs."""
        def _fi(off):
            d = {"burstArrivalTimeOffset": int(off)}
            if adjusted_periodicity_us is not None:
                d["adjustedPeriodicity"] = int(adjusted_periodicity_us)
            return d
        fb = {}
        if bat_offset_us_dl is not None:
            fb["tSCFeedbackInformationDL"] = _fi(bat_offset_us_dl)
        if bat_offset_us_ul is not None:
            fb["tSCFeedbackInformationUL"] = _fi(bat_offset_us_ul)
        with _lock:
            xfer = NGAP.NGAP_IEs.PDUSessionResourceNotifyTransfer
            xfer.set_val({"qosFlowNotifyList": [{
                "qosFlowIdentifier": int(qfi),
                "notificationCause": "fulfilled",
                "iE-Extensions": [{
                    "id": 393, "criticality": "ignore",
                    "extensionValue": ("TSCTrafficCharacteristicsFeedback", fb),
                }],
            }]})
            transfer_bytes = xfer.to_aper()
        return NgapCodec.encode(('initiatingMessage', {
            "procedureCode": 30, "criticality": "ignore",
            "value": ("PDUSessionResourceNotify", {"protocolIEs": [
                {"id": 10, "criticality": "reject", "value": ("AMF-UE-NGAP-ID", amf_ue_ngap_id)},
                {"id": 85, "criticality": "reject", "value": ("RAN-UE-NGAP-ID", ran_ue_ngap_id)},
                {"id": 66, "criticality": "reject", "value": ("PDUSessionResourceNotifyList", [{
                    'pDUSessionID': pdu_session_id,
                    'pDUSessionResourceNotifyTransfer': transfer_bytes,
                }])},
            ]}),
        }))

...
class NasBuilder:
    ...
    @staticmethod
    def pdu_session_estab_request(pdu_session_id, pti, pdu_session_type=1,
                                  dstt_mac=None, residence_time_ns=None, pmic=None):
        """Build the inner 5GSM PDU SESSION ESTABLISHMENT REQUEST bytes
        (TS 24.501 §8.3.1), optionally with the Rel-19 DS-TT IEs."""
        pdu_req = FGSMPDUSessionEstabRequest()
        pdu_req['5GSMHeader']['PDUSessID'].set_val(pdu_session_id)
        pdu_req['5GSMHeader']['PTI'].set_val(pti)
        pdu_req['PDUSessType']['V'].set_val(pdu_session_type)  # 1=IPv4, 5=Ethernet
        pdu_req['IntegrityProtMaxDataRate']['V'].set_val(b'\xff\xff')
        if dstt_mac is not None:
            pdu_req['DSTTEthernetMAC']['V'].set_val(bytes(dstt_mac))
            pdu_req['DSTTEthernetMAC'].set_trans(False)
        if residence_time_ns is not None:
            # §9.11.4.26: IEEE 1588 correctionField (ns × 2^16), 8 octets.
            pdu_req['UEDSTTResidenceTime']['V'].set_val(
                (int(residence_time_ns) << 16).to_bytes(8, 'big'))
            pdu_req['UEDSTTResidenceTime'].set_trans(False)
        if pmic is not None:
            pdu_req['PortMgmtInfoContainer']['V'].set_val(bytes(pmic))
            pdu_req['PortMgmtInfoContainer'].set_trans(False)
        return pdu_req.to_bytes()

    @staticmethod
    def ul_nas_transport_pdu_session(pdu_session_id, pti, dnn, sst, sd=None,
                                     pdu_session_type=1, dstt_mac=None,
                                     residence_time_ns=None, pmic=None):
        """TS 24.501 §8.2.10 — UL NAS Transport with PDU Session Estab Request.

        Rel-19 TSN (TS 23.501 §5.27): for Ethernet PDU sessions with a
        DS-TT (pdu_session_type=5) the UE may include the DS-TT
        Ethernet port MAC (IEI 0x6E, §9.11.4.25), the UE-DS-TT
        residence time (IEI 0x6F, §9.11.4.26 — IEEE 1588 correctionField
        ns×2^16) and an initial Port management information container
        (IEI 0x74, §9.11.4.27).
        """
        gsm_pdu = NasBuilder.pdu_session_estab_request(
            pdu_session_id, pti, pdu_session_type=pdu_session_type,

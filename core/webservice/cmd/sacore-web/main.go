...
import (
	...
	smfsession "github.com/mmt/mmt-studio-core/nf/smf/session"
...
func main() {
	...
	//
	// The UPF NF is DPDK/cgo-linked, so the collectors package does not
	// import it directly; wire the live IOStats in here (we already link
	// UPF) before starting the collection loop. AMF/SMF/PM are pure Go
	// and read by the collectors directly.
	wireNWDAFCollectors()
	nwdaf.DefaultService.Start()

	// NWDAF exposure (AF-via-NEF, TS 23.288 §6.1.1.2 subscribe/notify):
	// compute notification payloads via the running NWDAF service, then
	// start the periodic subscription notifier. Without this the exposure
	// subscriptions would never deliver (the loop was never started) and
	// would carry empty results.
	exposure.SetAnalyticsProvider(func(id, imsi, dnn string, windowSec int) any {
		return nwdaf.DefaultService.GetAnalytics(id, imsi, dnn, windowSec)
	})
	exposure.ExposureSubMgr.Start()

	// Rel-19 TSC control plane: TSN AF (TS 23.501 §5.28, CNC-facing)
	// and TSCTSF (TS 29.565, time sync / TSC QoS exposure). Both
	// subscribe to PCF TSC user plane events (TSN_BRIDGE_INFO /
	// BAT_OFFSET_INFO) at Start; REST surfaces live at /api/tsnaf/*,
	// /api/tsn/cnc/* and /api/tsctsf/*.
	tsnaf.Default.Start()
	tsctsf.Default.Start()
	...
	lifecycle.Register("nwdaf-exposure", func(context.Context) { exposure.ExposureSubMgr.Stop() })
...
// wireNWDAFCollectors injects the live AMF/SMF/UPF state accessors into
// the NWDAF collectors package. The collectors package stays free of NF
// imports (the SMF/UPF stores transitively pull the DPDK dataplane) so
// its unit tests stay pure-Go; the composition root — which already links
// every NF — supplies the concrete readers here. Must run before
// nwdaf.DefaultService.Start() so the first collection tick sees them.
func wireNWDAFCollectors() {
	collectors.SetAMFStatsProvider(func() collectors.AMFStats {
		total := uectx.Default.Count()
		connected := 0
		for _, ue := range uectx.Default.Snapshot() {
			if ue.CM == uectx.CMConnected {
				connected++
			}
		}
		idle := total - connected
		if idle < 0 {
			idle = 0
		}
		return collectors.AMFStats{
			TotalUEs:   total,
			Registered: uectx.Default.RegisteredCount(),
			Connected:  connected,
			Idle:       idle,
		}
	})

	collectors.SetSMFStatsProvider(func() collectors.SMFStats {
		byDNN := map[string]int{}
		bySlice := map[string]int{}
		active := 0
		for _, sess := range smfsession.Default.All() {
			if sess.State != smfsession.StateActive {
				continue
			}
			active++
			byDNN[sess.DNN]++
			bySlice[strconv.Itoa(int(sess.SST))]++
		}
		return collectors.SMFStats{
			ActiveSessions:  active,
			SessionsByDNN:   byDNN,
			SessionsBySlice: bySlice,
			IPPoolUsage:     ipalloc.Default.Usage(),
		}
	})

	collectors.SetUPFStatsProvider(func() collectors.UPFStats {
		st := upf.Default.GetIOStats()
		return collectors.UPFStats{
			RxPkts:         st.ULPkts,
			TxPkts:         st.DLPkts,
			RxBytes:        st.ULBytes,
			TxBytes:        st.DLBytes,
			Dropped:        st.ULDropped + st.DLDropped,
			ActiveSessions: upf.Default.SessionCount(),
		}
	})
}

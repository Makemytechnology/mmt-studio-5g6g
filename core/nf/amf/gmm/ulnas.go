...
func handleN1SMPayload(ue *uectx.AmfUeCtx, ul *nas.ULNASTransport, gsm []byte) {
	...
	case *nas.PDUSessionEstablishmentRequest:
		...
		// Rel-19 TSC IEs (TS 24.501 §9.11.4.25-27) — present on
		// Ethernet PDU sessions established by a UE hosting a DS-TT
		// (TS 23.502 §4.3.2.2.1). Passed through opaquely; the SMF
		// owns decode + the TSN_BRIDGE_INFO report to the PCF.
		var dsttMAC, residence, pmic []byte
		if sm.DSTTEthernetPortMACAddress != nil {
			dsttMAC = sm.DSTTEthernetPortMACAddress.Value
		}
		if sm.UEDSTTResidenceTime != nil {
			residence = sm.UEDSTTResidenceTime.Value
		}
		if sm.PortManagementInformationContainer != nil {
			pmic = sm.PortManagementInformationContainer.Value
		}
	...
	case *nas.PDUSessionModificationComplete:
		...
		// Rel-19 TSC: DS-TT reply PMIC (TS 24.501 §8.3.10 IE 0x74)
		// travels back to the TSN AF/TSCTSF via SMF → PCF
		// (TS 23.501 §5.28.3.2 DS-TT→AF path).
		if sm.PortManagementInformationContainer != nil {
			session.HandleModificationCompletePMIC(ue.IMSI, pduSessionID,
				sm.PortManagementInformationContainer.Value)
		}

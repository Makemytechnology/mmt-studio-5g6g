...
func Handle(gnb *gnbctx.GnbCtx, env *wire.Envelope, stream int) {
	...
	// §8.7.1.1 — a gNB re-associating under the same Global RAN Node
	// ID supersedes its prior association (UE contexts released,
	// old conn closed). Identity is the gNB ID, not the source IP.
	ngap.SupersedeGnbByID(id, gnb, "ng-setup-reassociation")

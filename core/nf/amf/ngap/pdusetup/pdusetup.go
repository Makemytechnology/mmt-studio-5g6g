...
func buildTransfer(sess *session.Session) ([]byte, error) {
	...
	// Rel-19 TSC: attach TSCTrafficCharacteristics (TS 38.413
	// §9.3.1.229; protocol extension id 196 of QosFlowSetupRequestItem)
	// for every QoS flow the SMF derived TSCAI for (TS 23.501
	// §5.27.2.4). TSC flows beyond the default ride the same list.
	for qfi, tscai := range sess.TSCAIByQFI {
		tc := BuildTSCTrafficCharacteristics(tscai[0], tscai[1])
		if tc == nil {
			continue
		}
		item := genngap.QosFlowSetupRequestItem{
			QosFlowIdentifier: genngap.QosFlowIdentifier(int64(qfi)),
			QosFlowLevelQosParameters: genngap.QosFlowLevelQosParameters{
				QosCharacteristics: genngap.QosCharacteristics{
					Present: genngap.QosCharacteristicsPresentNonDynamic5QI,
					NonDynamic5QI: &genngap.NonDynamic5QIDescriptor{
						FiveQI: genngap.FiveQI(int64(tscFiveQIFor(sess, qfi))),
					},
				},
				AllocationAndRetentionPriority: genngap.AllocationAndRetentionPriority{
					PriorityLevelARP:        genngap.PriorityLevelARP(2),
					PreEmptionCapability:    genngap.PreEmptionCapabilityShallNotTriggerPreEmption,
					PreEmptionVulnerability: genngap.PreEmptionVulnerabilityNotPreEmptable,
				},
			},
		}
		ext := []genngap.QosFlowSetupRequestItemExtIEsEntry{{
			Id:          genngap.ProtocolExtensionID(genngap.IdTSCTrafficCharacteristics),
			Criticality: genngap.CriticalityIgnore,
			Value: genngap.QosFlowSetupRequestItemExtIEsValue{
				Present:                   genngap.QosFlowSetupRequestItemExtIEsValuePresentTSCTrafficCharacteristics,
				TSCTrafficCharacteristics: tc,
			},
		}}
		if qfi == 1 {
			qos[0].IEExtensions = &ext
			continue
		}
		item.IEExtensions = &ext
		qos = append(qos, item)
	}
...
// BuildTSCTrafficCharacteristics maps the SMF-derived TSCAI (TS 23.501
// §5.27.2.4) onto the NGAP TSCTrafficCharacteristics IE (TS 38.413):
// Periodicity in µs (0..640000) and Burst Arrival Time as the 4-octet
// APER-independent opaque of §9.3.1.229 (here: 64-bit ns timestamp,
// big-endian, per the RAN's reference time encoding).
func BuildTSCTrafficCharacteristics(ul, dl *session.TSCAI) *genngap.TSCTrafficCharacteristics {
	conv := func(t *session.TSCAI) *genngap.TSCAssistanceInformation {
		if t == nil || t.PeriodicityUS == 0 {
			return nil
		}
		p := t.PeriodicityUS
		if p > 640000 {
			p = 640000 // NGAP Periodicity upper bound (extensible)
		}
		ai := &genngap.TSCAssistanceInformation{Periodicity: genngap.Periodicity(p)}
		if t.BurstArrivalTimeNS != 0 {
			bat := make(genngap.BurstArrivalTime, 8)
			binary.BigEndian.PutUint64(bat, uint64(t.BurstArrivalTimeNS))
			ai.BurstArrivalTime = &bat
		}
		return ai
	}
	tc := &genngap.TSCTrafficCharacteristics{
		TSCAssistanceInformationUL: conv(ul),
		TSCAssistanceInformationDL: conv(dl),
	}
	if tc.TSCAssistanceInformationUL == nil && tc.TSCAssistanceInformationDL == nil {
		return nil
	}
	return tc
}

// tscFiveQIFor resolves the 5QI of a TSC QoS flow from the session's
// rule→QFI map; defaults to 82 (delay-critical GBR) when unknown.
func tscFiveQIFor(sess *session.Session, qfi uint8) int {
	if qfi == 1 {
		if sess.FiveQI != 0 {
			return int(sess.FiveQI)
		}
		return 9
	}
	// The SMF QoS encoder allocated this QFI for a PCC rule; the 5QI
	// travelled in the NAS flow description. 82 is the safe delay-
	// critical default when the lookup is unavailable here.
	return 82
}

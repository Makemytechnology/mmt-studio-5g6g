...
// SupersedeGnbByID implements TS 38.413 §8.7.1.1: a successful NG
// Setup "re-initialises the NGAP UE-related contexts (if any) and
// erases all related signalling connections in the two nodes". When a
// gNB re-associates (new SCTP association, same Global RAN Node ID),
// the prior association's UE state is cascade-released, its registry
// entries cleared, and its conn closed so the stale accept goroutine
// unwinds (gnbctx.IsSuperseded short-circuits its deferred cleanup).
// Called from ngsetup with the new association's context excluded.
func SupersedeGnbByID(gnbID string, keep *gnbctx.GnbCtx, reason string) {
	if gnbID == "" {
		return
	}
	for _, old := range gnbctx.Default.All() {
		if old == keep || old.GnbID != gnbID {
			continue
		}
		logger.Get("amf.ngap.server").Warnf(
			"gNB id=%s: new association %s supersedes prior %s (TS 38.413 §8.7.1.1, %s)",
			gnbID, keep.GnbIP, old.GnbIP, reason)
		cascadeNGResetForGnb(old.GnbIP, reason)
		uectx.Default.RemoveAllForGnb(old.GnbIP)
		gnbctx.Default.Remove(old.GnbIP)
		old.Supersede()
	}
}

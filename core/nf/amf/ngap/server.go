...
func (s *Server) acceptLoop(ctx context.Context) {
	...
	for {
		...
		// Key the context by the full SCTP association endpoint
		// (ip:port), not the bare IP: TS 38.412 identifies the NG-C
		// TNL association, and multiple gNBs may legitimately share a
		// host IP (test benches, containerised RAN). The §8.7.1.1
		// "re-initialise on new association from the same gNB"
		// semantic moved to NG Setup, where the gNB is identified by
		// its Global RAN Node ID (ngsetup → SupersedeGnbByID) — IP was
		// never the gNB's identity.
		ip := conn.RemoteAddr()

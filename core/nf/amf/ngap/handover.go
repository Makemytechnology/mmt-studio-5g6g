...
func handleHandoverPreparation(source *gnbctx.GnbCtx, env *wire.Envelope, stream int) {
	...
	// Preferred path: build a spec-complete §9.2.3.4 HANDOVER REQUEST
	// from the live UE + SMF session view. When no UE context / SMF
	// session is available (AMF acting as a pure N2 relay, or a
	// capture-replay), fall back to relaying the Source-to-Target
	// transparent container — the AMF forwards it unchanged either way
	// (§8.4.2), so the handover orchestration (prep → ack → command,
	// status-transfer relay, notify → release) still runs. The relay
	// fallback is logged so a missing UE context is visible.
	var hoReqValue []byte
	if ue := uectx.Default.LookupByAmfID(amfUEID); ue != nil {
		if v, err := BuildHandoverRequestValue(ue, parsedReq, env.Value); err == nil {
			hoReqValue = v
		} else {
			log.Warnf("HandoverRequired from %s: build HANDOVER REQUEST failed (%v) — relaying transparent container",
				source.GnbIP, err)
		}
	} else {
		log.Infof("HandoverRequired from %s: amfUeID=%d has no UE context — relaying transparent container to target",
			source.GnbIP, amfUEID)
	}
	if hoReqValue == nil {
		// Transparent relay: forward the HANDOVER REQUIRED value to the
		// target as the HANDOVER REQUEST. The Source-to-Target
		// container (IE 101) is common to both messages and is what the
		// target actually consumes.
		hoReqValue = env.Value
	}

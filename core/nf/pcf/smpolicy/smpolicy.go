...
type SmPolicyDecision struct {
	...
	// ── TSC management provisioning (TS 29.512 §5.6.2.4, feature
	//    TimeSensitiveNetworking; §4.2.3.24 "Provisioning of TSCAI
	//    input information and TSC QoS related data") ──
	// tsnPortManContDstt — PMIC destined for the DS-TT port: the SMF
	// relays it in a PDU SESSION MODIFICATION COMMAND (TS 24.501
	// §9.11.4.27). One-shot: consumed by the SMF on delivery.
	TsnPortManContDstt *pcf.PortManCont
	// tsnPortManContNwtts — PMICs destined for NW-TT ports: relayed
	// over N4 in TSC Management Information (TS 29.244 IE 199).
	TsnPortManContNwtts []pcf.PortManCont
	// tsnBridgeManCont — UMIC for the NW-TT (TS 29.512 §5.6.2.47).
	TsnBridgeManCont []byte
}
...
type SmPolicyContextDataUpdate struct {
	...
	// ── TSC user plane node reporting (TS 29.512 §5.6.2.19, trigger
	//    TSN_BRIDGE_INFO per §4.2.4.15) ──
	// tsnBridgeInfo — bridge ID / DS-TT port info learned from the UPF
	// (N4 Created Bridge Info for TSC) and the UE (N1 DS-TT IEs).
	TsnBridgeInfo *pcf.TsnBridgeInfo
	// tsnPortManContDstt — PMIC received from the DS-TT via N1.
	TsnPortManContDstt *pcf.PortManCont
	// tsnPortManContNwtts — PMIC(s) received from NW-TT ports via N4.
	TsnPortManContNwtts []pcf.PortManCont
	// tsnBridgeManCont — UMIC received from the NW-TT via N4.
	TsnBridgeManCont []byte
	// BatOffsets — NG-RAN BAT offset feedback per QoS flow / rule
	// (TS 23.501 §5.27.2.5, EnTSCAC event BAT_OFFSET_INFO).
	BatOffsets []pcf.BatOffsetInfo
	// UeMac — trigger UE_MAC_CH (§5.6.3.6): a UE MAC address detected
	// or removed on an Ethernet PDU session ("aa-bb-cc-dd-ee-ff").
	UeMac string
}

// TriggerTsnBridgeInfo is the TS 29.512 §5.6.3.6 PolicyControlRequestTrigger
// value the SMF reports when new TSC user plane node information, PMIC(s)
// or UMIC arrive (§4.2.4.15).
const TriggerTsnBridgeInfo = "TSN_BRIDGE_INFO"
...
type association struct {
	...
	createdAt time.Time
	// tsnDsttMac — DS-TT port MAC from the latest TSN_BRIDGE_INFO
	// report (TS 29.512 §5.6.2.41 dsttAddr); lets the TSN AF address
	// this association by MAC (TS 23.501 §5.28.3.2).
	tsnDsttMac string
}
...
func Update(k smfsm.Key, upd SmPolicyContextDataUpdate) (SmPolicyDecision, error) {
	...
	// TSN_BRIDGE_INFO trigger (§4.2.4.15) / BAT offset feedback: fan the
	// TSC user plane report out to the TSN AF / TSCTSF listeners as an
	// Npcf_PolicyAuthorization Notify (TS 29.514 §5.6.3.7 events
	// TSN_BRIDGE_INFO, BAT_OFFSET_INFO).
	noteTsnDsttMac(a, upd)
	if upd.TsnBridgeInfo != nil || upd.TsnPortManContDstt != nil ||
		len(upd.TsnPortManContNwtts) > 0 || len(upd.TsnBridgeManCont) > 0 ||
		len(upd.BatOffsets) > 0 || upd.UeMac != "" {
		pcf.NotifyTscUplaneEvent(pcf.TscUplaneEvent{
			IMSI:             a.key.IMSI,
			PDUSessionID:     a.key.PDUSessionID,
			DNN:              a.ctxData.DNN,
			SNSSAISST:        a.ctxData.SST,
			BridgeInfo:       upd.TsnBridgeInfo,
			PortManContDstt:  upd.TsnPortManContDstt,
			PortManContNwtts: upd.TsnPortManContNwtts,
			BridgeManCont:    upd.TsnBridgeManCont,
			BatOffsets:       upd.BatOffsets,
			DetectedMac:      upd.UeMac,
		})
	}
...
// PushTscManagement ships PMIC(s)/UMIC from the TSN AF / TSCTSF down to
// the SMF for delivery to the DS-TT (N1 PDU Session Modification, PMIC IE
// TS 24.501 §9.11.4.27) or the NW-TT (N4 TSC Management Information,
// TS 29.244 IE 199) — TS 29.512 §4.2.3.24 provisioning via UpdateNotify.
//
// The containers are one-shot: they ride on a copy of the current
// decision and are not retained in the association.
func PushTscManagement(k smfsm.Key, dstt *pcf.PortManCont, nwtts []pcf.PortManCont, umic []byte) error {
	assocMu.RLock()
	a := assocs[k]
	assocMu.RUnlock()
	if a == nil {
		return fmt.Errorf("pcf.smpolicy: no association for %s", k)
	}
	decision := a.decision // copy
	decision.TsnPortManContDstt = dstt
	decision.TsnPortManContNwtts = nwtts
	decision.TsnBridgeManCont = umic
	return PushNotify(k, decision)
}

// FindAssociationByIMSIDNN locates the SM Policy association of a
// UE's PDU session on the given DNN — the TSCTSF resolves target UEs
// to AF-session bindings this way (TS 29.565: SUPI + DNN/S-NSSAI in
// TimeSyncExposureSubsc / TscAppSessionContextData).
func FindAssociationByIMSIDNN(imsi, dnn string) (smfsm.Key, bool) {
	assocMu.RLock()
	defer assocMu.RUnlock()
	for k, a := range assocs {
		if k.IMSI == imsi && a.ctxData.DNN == dnn {
			return k, true
		}
	}
	return smfsm.Key{}, false
}

// FindAssociationByDsttMac locates the SM Policy association whose PDU
// session reported the given DS-TT MAC in its TsnBridgeInfo — the TSN AF
// addresses ports by MAC (TS 23.501 §5.28.3.2: "the TSN AF provides the
// PMIC and the DS-TT port MAC address"). Returns the zero Key when the
// MAC is unknown.
//
// The MAC↔association binding is kept by the TSN AF itself (it received
// the TsnBridgeInfo event); this helper only exists for callers that
// track MACs but not keys.
func FindAssociationByDsttMac(mac string) (smfsm.Key, bool) {
	assocMu.RLock()
	defer assocMu.RUnlock()
	for k, a := range assocs {
		if a.tsnDsttMac == mac {
...
// NoteTsnDsttMac records the DS-TT MAC that a session's bridge report
// carried, enabling FindAssociationByDsttMac. Called from Update.
func noteTsnDsttMac(a *association, upd SmPolicyContextDataUpdate) {
	if upd.TsnBridgeInfo != nil && upd.TsnBridgeInfo.DsttAddr != "" {
		a.tsnDsttMac = upd.TsnBridgeInfo.DsttAddr
	}
}

...
type PCCRule struct {
	...
	// ── TSC attributes (TS 29.512 §5.6.2.6, feature
	//    TimeSensitiveNetworking / TimeSensitiveCommunication) ──
	// tscaiInputUl / tscaiInputDl — TSCAI input containers the SMF
	// turns into NG-RAN TSCAI per TS 23.501 §5.27.2.4.
	TscaiInputUl *TscaiInput
	TscaiInputDl *TscaiInput
	// tscaiTimeDom — (g)PTP domain of the (TSN) AF (0 = 5GS clock).
	TscaiTimeDom uint64
	// capBatAdaptation — AF adapts burst arrival on RAN BAT offsets
	// (EnTSCAC).
	CapBatAdaptation bool
	// ── QosData additions used for TSC flows (TS 29.512 §5.6.2.8) ──
	// maxDataBurstVol — MDBV in octets (delay-critical GBR only;
	// TS 23.501 §5.27.3 sets it to the aggregated TSC burst size).
	MaxDataBurstVol int
	// packetDelayBudget of the selected 5QI in milliseconds — the SMF
	// splits this into 5G-AN PDB + CN PDB for the TSCAI BAT shift.
	PacketDelayBudgetMS float64
	// ── Ethernet flow description (Ethernet PDU sessions;
	//    TS 29.514 EthFlowDescription → PDI Ethernet Packet Filter) ──
	// EthDestMAC — destination MAC ("aa-bb-cc-dd-ee-ff"); the SMF
	// installs a PDR with the §8.2.93 MAC filter for it.
	EthDestMAC string
	// EthVLANID — C-TAG VID (IEEE 802.1Q), 0 = untagged.
	EthVLANID int
}
...
func CreatePolicy(imsi, dnn string, sst uint8) PCCRuleSet {
	...
	// Dynamic TSC rules installed by the TSN AF / TSCTSF via
	// Npcf_PolicyAuthorization (TS 23.503 §6.1.3.23/23a) ride on top of
	// the DB-backed bindings — they exist only for the AF session
	// lifetime and are never persisted.
	rules = append(rules, TscPccRules(imsi, dnn)...)

...
// ── upf.TSCBridge delegation (Rel-19 TSC, see pfcp_tsc.go) ──

func (r *RouterBridge) RequestBridgeInfo(imsi string, pduSessionID uint8) error {
	br, err := r.dispatch(imsi, pduSessionID)
	if err != nil {
		return err
	}
	return br.RequestBridgeInfo(imsi, pduSessionID)
}

func (r *RouterBridge) BridgeInfo(imsi string, pduSessionID uint8) (uint16, []uint16, uint64, bool) {
	br, err := r.dispatch(imsi, pduSessionID)
	if err != nil {
		return 0, nil, 0, false
	}
	return br.BridgeInfo(imsi, pduSessionID)
}

func (r *RouterBridge) SendTSCManagementInformation(imsi string, pduSessionID uint8,
	entries []upf.TSCMgmtEntry) ([]upf.TSCMgmtEntry, error) {
	br, err := r.dispatch(imsi, pduSessionID)
	if err != nil {
		return nil, err
	}
	return br.SendTSCManagementInformation(imsi, pduSessionID, entries)
}

...
type pendingRules struct {
	...
	// wantBridgeInfo — arm Create Bridge Info for TSC (IE 194, BII=1)
	// on the Establishment (Rel-19 TSC; set via RequestBridgeInfo).
	wantBridgeInfo bool
}
...
type pendingPDR struct {
	...
	// Rel-19 Ethernet PDU sessions — §7.5.2.2 PDI Ethernet Packet
	// Filter (destination MAC §8.2.93 + C-TAG VID §8.2.94).
	ethDestMAC []byte // 6 octets when set
	ethVLANID  uint16 // 0 = none
	teid   uint32
	n3IPv4 uint32
}
...
type sessionState struct {
	...
	// Created Bridge Info for TSC (§7.5.3.6, Rel-19) from the
	// Establishment Response — see pfcp_tsc.go.
	hasBridgeInfo   bool
	bridgeDSTTPort  uint16
	bridgeNWTTPorts []uint16
	bridgeNodeID    uint64
}
...
func (p *PfcpBridge) setupAssociation() error {
	req := &genpfcp.AssociationSetupRequest{
		...
		// Rel-19 TSC — §7.4.4.1 Clock Drift Control Information
		// (IE 203): request the UP function to report the time
		// offset and cumulative rateRatio between the 5GS clock and
		// every (g)PTP time domain it observes (RRTO|RRCR, §8.2.145).
		// The SMF folds the reports into the §5.27.2.4 TSCAI
		// derivation (nf/smf/session/tsc.go ApplyClockDriftReport).
		ClockDriftControlInformation: []genpfcp.ClockDriftControlInformation{{
			RequestedClockDriftInformation: genpfcp.RequestedClockDriftInformation{
				Value: []byte{0x03}, // RRTO(1) | RRCR(2)
			},
		}},
	}
	payload, err := stripHeader(req)
...
func (p *PfcpBridge) CommitSession(imsi string, pduSessionID uint8) error {
	...
	// §7.5.2.1 Create Bridge Info for TSC (IE 194) — BII=1 requests
	// the UPF to allocate a DS-TT port and report the 5GS user plane
	// node (bridge) identity (Rel-19 TSC, TS 23.502 §4.3.2.2.1 10a).
	if pend.wantBridgeInfo {
		req.CreateBridgeInfoForTSC = &genpfcp.CreateBridgeInfoForTSC{Value: []byte{0x01}}
	}
	...
	p.mu.Lock()
	st := &sessionState{cpSEID: pend.cpSEID, upSEID: upSEID}
	// §7.5.3.6 Created Bridge Info for TSC (Rel-19) — DS-TT port,
	// NW-TT port(s) and 5GS User Plane Node ID from the UPF.
	st.applyCreatedBridgeInfo(resp.CreatedBridgeInfoForTSC)
	p.sessions[key] = st
	...
	if !established {
		...
		for _, x := range batch.CreatePDRs {
			r.pdrs = append(r.pdrs, pendingPDR{
				...
				ethDestMAC: x.EthDestMAC, ethVLANID: x.EthVLANID,
			})
		}
...
func (p *PfcpBridge) handleInbound(hdr *runtime.Header, payload []byte, peer *net.UDPAddr) {
	// §7.4.5.1 Node Report Request — Rel-19 clock drift reports
	// (CKDR): the UP function reports the offset between the 5GS
	// clock and external (g)PTP time domains that we requested at
	// Association Setup (Clock Drift Control Information).
	if hdr.MessageType == genpfcp.MessageTypeNodeReportRequest {
		p.handleNodeReport(hdr, payload, peer)
		return
	}
	...
	if known && req.ReportType.DLDR == 1 && req.DownlinkDataReport != nil {
		...
		// §7.5.8.3 Usage Report. Rel-19 Ethernet PDU sessions carry
		// Ethernet Traffic Information (§8.2.98 MAC Addresses
		// Detected) here — decode and hand to the SMF so it can fire
		// the UE_MAC_CH policy trigger (TS 29.512 §5.6.3.6).
		// Volume/charging measurements remain unconsumed (no CHF
		// consumer yet).
		for i := range req.UsageReport {
			eti := req.UsageReport[i].EthernetTrafficInformation
			if eti == nil {
				continue
			}
			var macs [][6]byte
			for j := range eti.MACAddressesDetected {
				macs = append(macs, decodeMACsDetected(eti.MACAddressesDetected[j].Value)...)
			}
			if len(macs) == 0 {
				continue
			}
			r := upf.Report{
				Type:         upf.ReportEthMAC,
				IMSI:         key.imsi,
				PDUSessionID: key.pduSessionID,
				SEID:         hdr.SEID,
				Timestamp:    time.Now(),
				EthMAC:       &upf.EthMACPayload{MACs: macs},
			}
			select {
			case p.reports <- r:
				p.log.WithIMSI(key.imsi).Infof("§8.2.98 MAC Addresses Detected: %d MAC(s) pduSessID=%d",
					len(macs), key.pduSessionID)
			default:
				p.reportsDropped.Add(1)
			}
		}
...
func buildCreatePDR(p pendingPDR) genpfcp.CreatePDR {
	...
	// Rel-19 §7.5.2.2 Ethernet Packet Filter (IE 132): destination
	// MAC (§8.2.93, DEST flag) + optional C-TAG VID (§8.2.94; octet6
	// bit8 = VID MSB, octet7 bit1 = VID LSB).
	if len(p.ethDestMAC) == 6 {
		mac := make([]byte, 7)
		mac[0] = 0x02 // DEST
		copy(mac[1:], p.ethDestMAC)
		epf := genpfcp.EthernetPacketFilter{
			MACAddress: []genpfcp.MACAddress{{Value: mac}},
		}
		if p.ethVLANID != 0 {
			vid := p.ethVLANID & 0x0FFF
			epf.CTAG = []genpfcp.CTAG{{Value: []byte{
				0x04, // VID flag
				byte(vid>>8) << 4,
				byte(vid),
			}}}
		}
		out.PDI.EthernetPacketFilter = []genpfcp.EthernetPacketFilter{epf}
	}
	if p.sdfRules != "" {

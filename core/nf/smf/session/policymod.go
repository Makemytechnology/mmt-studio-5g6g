...
func handlePolicyUpdateNotify(k smfsm.Key, decision smpolicy.SmPolicyDecision) {
	...
	for i, fs := range flowSpecs {
		...
		// Rel-19 Ethernet PDU sessions: a TSC rule carrying an
		// Ethernet flow description installs a DL PDR with the
		// §7.5.2.2 PDI Ethernet Packet Filter (destination MAC +
		// C-TAG VID) so the UPF/NW-TT learns the stream MAC
		// (§8.2.98 reporting closes the loop back to the TSN AF).
		if sess.PDUType == nas.PDUSessionTypeEthernet {
			if mac, vlan := pccEthFilter(decision.PccRules, fs.FiveQI); mac != nil {
				batch.CreatePDRs = append(batch.CreatePDRs, upfmgr.PDR{
					PDRID:      100 + uint16(fs.QFI),
					Precedence: 100, // above the default-flow PDRs
					PDISource:  1,   // core (DL)
					QFI:        fs.QFI,
					FARID:      2, // default DL FAR until per-QFI TNL known
					QERID:      100 + uint32(fs.QFI),
					EthDestMAC: mac,
					EthVLANID:  uint16(vlan),
				})
				log.Infof("PFCP §7.5.4 batched: Ethernet PDR QFI=%d destMAC=%02x-%02x-%02x-%02x-%02x-%02x vlan=%d",
					fs.QFI, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], vlan)
			}
		}
	...
	// Rel-19 TSC (TS 29.512 §4.2.3.24): AF-originated PMIC for the
	// DS-TT rides the §8.3.9 command inside the Port management
	// information container IE (TS 24.501 §9.11.4.27); NW-TT PMICs
	// and the UMIC go over N4 TSC Management Information instead
	// (TS 29.244 §7.5.4.18; see forwardTscManagementToUserPlane).
	if decision.TsnPortManContDstt != nil && len(decision.TsnPortManContDstt.Container) > 0 {
		cmd.PortManagementInformationContainer = &nas.PortManagementInformationContainer{
			Value: decision.TsnPortManContDstt.Container,
		}
		log.Infof("UpdateNotify: DS-TT PMIC (%d B) attached to §8.3.9 (TS 24.501 §9.11.4.27)",
			len(decision.TsnPortManContDstt.Container))
	}
	if len(decision.TsnPortManContNwtts) > 0 || len(decision.TsnBridgeManCont) > 0 {
		forwardTscManagementToUserPlane(sess, decision.TsnPortManContNwtts, decision.TsnBridgeManCont)
	}

	// Refresh the per-QFI TSCAI table so the NGAP path (AMF pdusetup /
	// resource modify) carries current TSCTrafficCharacteristics
	// (TS 23.501 §5.27.2.4).
	RefreshSessionTSCAI(sess, decision.PccRules)

	// N2 leg (TS 23.502 §4.3.3 step 3): for TSC flows added after
	// establishment the gNB has not seen their TSCAI (it only rode the
	// establishment PDU Session Resource Setup). Send a PDU Session
	// Resource Modify Request carrying TSCTrafficCharacteristics so the
	// RAN can schedule the deterministic grant. The N1 leg (NAS to UE)
	// still ships below via DLNASByIMSI.
	if len(sess.TSCAIByQFI) > 0 && NGAPModifyTSCByIMSI != nil {
		if err := NGAPModifyTSCByIMSI(k.IMSI, k.PDUSessionID, sess.TSCAIByQFI); err != nil {
			log.Warnf("UpdateNotify: N2 TSC modify: %v", err)
		}
	}

	// Nothing to tell the UE? (pure NW-TT/UMIC push) — skip the NAS leg.
	if len(allRuleSpecs) == 0 && cmd.PortManagementInformationContainer == nil &&
		decision.SessionAMBRDL == 0 && decision.SessionAMBRUL == 0 {
		return
	}
...
// pccEthFilter returns the Ethernet flow description (destination MAC
// bytes + VLAN ID) of the PCC rule matching fiveQI, or (nil, 0).
func pccEthFilter(rules []pcf.PCCRule, fiveQI uint8) ([]byte, int) {
	for _, r := range rules {
		if r.FiveQI != int(fiveQI) || r.EthDestMAC == "" {
			continue
		}
		mac := parseMAC(r.EthDestMAC)
		if mac == nil {
			return nil, 0
		}
		return mac, r.EthVLANID
	}
	return nil, 0
}

// parseMAC accepts "aa-bb-cc-dd-ee-ff" / "aa:bb:..." forms.

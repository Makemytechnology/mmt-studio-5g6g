...
type Session struct {
	...
	// ── TSC / 5GS-bridge state (TS 23.501 §5.27/§5.28, Rel-19) ──
	// DSTTMAC — DS-TT Ethernet port MAC from the NAS establishment
	// request (TS 24.501 §9.11.4.25), "aa-bb-cc-dd-ee-ff" form.
	DSTTMAC string
	// UEDSTTResidenceNS — UE-DS-TT residence time (§9.11.4.26,
	// IEEE 1588 correctionField → nanoseconds). 0 = not provided.
	UEDSTTResidenceNS uint64
	// PendingPMIC — PMIC from the establishment request (§9.11.4.27),
	// held until the bridge report ships it to the PCF.
	PendingPMIC []byte
	// TSCBridgeID / DSTTPortNum / NWTTPortNums — Created Bridge Info
	// from the UPF (TS 29.244 IE 195; TS 23.502 §4.3.2.2.1 step 10b).
	TSCBridgeID  uint64
	DSTTPortNum  uint16
	NWTTPortNums []uint16
	// TSCAIByQFI — derived TSC Assistance Information per QoS flow
	// ([0]=UL, [1]=DL; TS 23.501 §5.27.2.4). Read by the AMF pdusetup
	// path to build the NGAP TSCTrafficCharacteristics extension.
	TSCAIByQFI map[uint8][2]*TSCAI
	// DetectedMACs — UE MACs learned by the UPF on this Ethernet PDU
	// session (§8.2.98 MAC Addresses Detected, reported over N4 and
	// forwarded to the PCF with trigger UE_MAC_CH).
	DetectedMACs []string
}

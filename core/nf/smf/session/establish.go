...
type EstablishInput struct {
	...
	// ── Rel-19 TSC IEs from the establishment request (Ethernet
	//    PDU sessions with a DS-TT; TS 23.502 §4.3.2.2.1) ──
	// DSTTMAC — DS-TT Ethernet port MAC address contents
	// (TS 24.501 §9.11.4.25, 6 octets). nil when absent.
	DSTTMAC []byte
	// UEDSTTResidenceTime — §9.11.4.26 contents (8 octets, IEEE 1588
	// correctionField: ns × 2^16). nil when absent.
	UEDSTTResidenceTime []byte
	// PMIC — §9.11.4.27 Port management information container from
	// the DS-TT, forwarded transparently toward the TSN AF/TSCTSF.
	PMIC []byte
}
...
func Establish(in EstablishInput) (*EstablishOutput, error) {
	...
	// Rel-19 TSC: fold the DS-TT IEs of the establishment request
	// into the session (TS 24.501 §9.11.4.25-27). The residence time
	// arrives encoded as an IEEE 1588 correctionField (ns × 2^16).
	if len(in.DSTTMAC) == 6 {
		sess.DSTTMAC = fmt.Sprintf("%02x-%02x-%02x-%02x-%02x-%02x",
			in.DSTTMAC[0], in.DSTTMAC[1], in.DSTTMAC[2],
			in.DSTTMAC[3], in.DSTTMAC[4], in.DSTTMAC[5])
	}
	if len(in.UEDSTTResidenceTime) == 8 {
		sess.UEDSTTResidenceNS = binary.BigEndian.Uint64(in.UEDSTTResidenceTime) >> 16
	}
	sess.PendingPMIC = in.PMIC
	...
	// Rel-19 TSC: for Ethernet PDU sessions arm the 5GS bridge
	// information request so the UPF allocates a DS-TT port and
	// reports the user plane node ID in the Establishment Response
	// (TS 23.502 §4.3.2.2.1 step 10a; TS 29.244 IE 194 BII=1).
	if sess.PDUType == nas.PDUSessionTypeEthernet {
		if err := upfmgr.Default.RequestBridgeInfo(sess.IMSI, sess.PDUSessionID); err != nil {
			log.WithIMSI(in.IMSI).Warnf("TSC bridge info request: %v", err)
		}
	}
	...
	// Rel-19 TSC: fold the Created Bridge Info (step 10b) into the
	// session and report TSN_BRIDGE_INFO + PMIC to the PCF → TSN AF /
	// TSCTSF (TS 29.512 §4.2.4.15). Derive initial TSCAI for any TSC
	// PCC rules already authorized (TS 23.501 §5.27.2.4).
	if sess.PDUType == nas.PDUSessionTypeEthernet {
		requestAndReportBridgeInfo(sess)
		RefreshSessionTSCAI(sess, policyDecision.PccRules)
	}
...
func ReleaseWithCause(imsi string, pduSessionID uint8, cause uint8, pti uint8) []byte {
	...
	// Rel-19 TSC: drop the DS-TT port (and 5GS bridge, when last) at
	// the TSN AF — the bridge exists per Ethernet PDU session
	// (TS 23.501 §5.28.1). No-op for non-Ethernet sessions.
	NotifyTSCSessionReleased(sess)

...
func (s *Service) IngestDataPoint(dp analytics.DataPoint) (int64, error) {
	...
	// Persist collected_at as the same unix-epoch float the collection
	// loop writes (not datetime('now')), so the column stays single-typed
	// and the caller's timestamp survives for historical replay.

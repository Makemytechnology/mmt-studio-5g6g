...
func (s *Service) Start() {
	...
	// Seed the in-memory cache from persisted data points so analytics
	// requests work immediately after a restart, rather than returning
	// "No data in time window" until the first collection tick.
	//
	// We already hold s.mu here (taken above), and sync.Mutex is NOT
	// reentrant, so we must call the *Locked variant — calling the
	// public rehydrateCache() would re-take s.mu and self-deadlock,
	// hanging Start() forever. Because main() calls Start() on the boot
	// goroutine, that hang blocks app.SetReady(), so the core never
	// reports ready=true and every reset-to-baseline times out.
	s.rehydrateCacheLocked()
	go s.collectionLoop()
...
//
// This is the SBI-facing subscription store (nwdaf_subscriptions) for
// internal 5GC NF consumers; processSubscriptions() delivers it. It is
// deliberately distinct from the NEF/AF-facing exposure subscriptions in
// nf/nwdaf/exposure (nwdaf_exposure_subscriptions, §6.1.1.2) — different
// consumers, auth model (API key + consent), and notifier loop.
//
...
func (s *Service) Subscribe(consumerNF, analyticsID string, targetIMSI, targetDNN, targetSST string,
	...
	// sub_id is UNIQUE; a bare second-precision timestamp collides when
	// two consumers subscribe within the same second, so append random
	// hex to keep IDs distinct under burst creation.
	rb := make([]byte, 6)
	_, _ = rand.Read(rb)
	subID := "nwdaf-sub-" + time.Now().Format("20060102150405") + "-" + hex.EncodeToString(rb)
...
// rehydrateCache seeds dataCache from the persisted nwdaf_data_points
// table on startup. It loads the most recent maxCachePoints rows per
// Analytics ID (newest-first from the DB, reversed into chronological
// order to match how collectAll appends), so GetAnalytics has data the
// moment the service starts instead of waiting a collection interval.
//
// This variant takes s.mu itself; callers that already hold s.mu (e.g.
// Start) must call rehydrateCacheLocked directly — sync.Mutex is not
// reentrant, so locking twice on the same goroutine self-deadlocks.
func (s *Service) rehydrateCache() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.rehydrateCacheLocked()
}

// rehydrateCacheLocked is rehydrateCache for callers that ALREADY hold
// s.mu. It must never take s.mu itself.
func (s *Service) rehydrateCacheLocked() {
	defer func() { recover() }()
	db, err := engine.Open()
	if err != nil {
		return
	}
	limit := s.maxCachePoints * len(analytics.ValidAnalyticsIDs)
	rows, err := db.Query(`SELECT source_nf, analytics_id, imsi, dnn, data_json, collected_at
		FROM nwdaf_data_points ORDER BY id DESC LIMIT ?`, limit)
	if err != nil {
		return
	}
	defer rows.Close()
	recs, _ := scanAllRows(rows)

	byID := make(map[string][]analytics.DataPoint)
	for _, r := range recs {
		aid := asString(r["analytics_id"])
		if aid == "" || len(byID[aid]) >= s.maxCachePoints {
			continue
		}
		byID[aid] = append(byID[aid], analytics.DataPoint{
			SourceNF:    asString(r["source_nf"]),
			AnalyticsID: aid,
			IMSI:        asString(r["imsi"]),
			DNN:         asString(r["dnn"]),
			DataJSON:    asString(r["data_json"]),
			CollectedAt: toEpoch(r["collected_at"]),
		})
	}

	// Caller holds s.mu (see the two callers above); do NOT re-lock.
	total := 0
	for aid, pts := range byID {
		// Rows arrived newest-first; reverse to oldest→newest.
	...
	if total > 0 {
		log.Infof("NWDAF rehydrated %d data points from DB across %d analytics IDs", total, len(byID))
	}
}
...
// asString coerces a SQL driver value (string or []byte) to a string.
...
// toEpoch coerces a collected_at column value to a unix-epoch float.
// The column has historically held both REAL epochs (collection loop)
// and datetime('now') TEXT (operator ingest), so handle both.
func toEpoch(v any) float64 {
	...
	case int:
		return float64(n)
	case string:
		return parseEpochString(n)
	case []byte:
		return parseEpochString(string(n))
	}
	return 0
}

// parseEpochString handles a collected_at value read back as text. The
// column has TEXT affinity, so a float epoch written by the collection
// loop / ingest path is stored as a numeric string ("1781961688"); the
// datetime('now') legacy default is stored as "2006-01-02 15:04:05".
func parseEpochString(s string) float64 {

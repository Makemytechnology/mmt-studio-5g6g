...
//
// User consent (TS 23.288 §6.2.9) is enforced on UE-scoped queries on
// both paths: the one-shot route and the subscription notifier
// (CheckAndNotify) gate on ConsentAllowed; CheckAnalyticsPermission
// additionally enforces the per-consumer analytics allow-list.
//
...
// analyticsProvider, when set, computes analytics for a subscription
// notification. Wired by the composition root to nwdaf.GetAnalytics so
// this package stays decoupled from the analytics service (and unit-tests
// without it). When unset, notifications carry a null result.
var analyticsProvider func(analyticsID, imsi, dnn string, windowSec int) any

// SetAnalyticsProvider registers the analytics compute source used by the
// background notifier. Called once at process startup.
func SetAnalyticsProvider(fn func(analyticsID, imsi, dnn string, windowSec int) any) {
...
func (m *SubscriptionManager) CheckAndNotify() {
	...
	for _, sub := range subs {
		...
		analyticsType := strVal(sub["analytics_type"])
		nowISO := now.Format(time.RFC3339)
		subID, _ := sub["id"].(int64)

		// Resolve the target scope. The one-shot path treats a slice
		// target as the DNN slot, so mirror that here.
		var imsi, dnn string
		switch strVal(sub["target_type"]) {
		case "imsi":
			imsi = strVal(sub["target_id"])
		case "slice":
			dnn = strVal(sub["target_id"])
		}

		// TS 23.288 §6.2.9 user-consent gate. UE-scoped notifications
		// must be consent-checked just like the one-shot exposure path;
		// on denial we suppress delivery, stamp last_notified (so a
		// denied sub isn't retried every tick), and audit-log a 403.
		if imsi != "" {
			if ok, reason := ConsentAllowed(consumerID, imsi); !ok {
				log.Debugf("Exposure notification suppressed: sub=%d consumer=%d imsi=%s: %s",
					subID, consumerID, imsi, reason)
				_ = UpdateSubscriptionNotified(subID, nowISO)
				LogQuery(&consumerID, analyticsType, "subscription", http.StatusForbidden)
				continue
			}
		}

		// Compute the analytics result so the notification carries data
		// (TS 23.288 §6.1.3). Resolve the stored Stage-3 type to an
		// internal Analytics ID before computing.
		var result any
		if analyticsProvider != nil {
			internalID := ExposureTypes[analyticsType]
			if internalID == "" {
				internalID = analyticsType
			}
			result = analyticsProvider(internalID, imsi, dnn, 300)
		}
		...
		code := 200
		if callbackURL != "" {
			code = postNotification(sub, result, callbackURL)
		}
...
// strVal coerces a SQL driver value (string or []byte) to a string.

...
type PDR struct {
	...
	// Rel-19 Ethernet PDU sessions — PDI Ethernet Packet Filter
	// (TS 29.244 §7.5.2.2 / IEs 132-135): destination MAC and
	// optional C-TAG VID the PDR matches. Empty/0 = no Ethernet
	// filter. Carried on the §7.5.4 Modification path for TSC
	// streams (nf/smf/session/policymod.go).
	EthDestMAC []byte // 6 octets when set
	EthVLANID  uint16 // C-TAG VID, 0 = none
}

...
func EnableMulti(specs []SliceSpec) (*State, error) {
	...
	// Rel-19 TSC: point the NW-TT's dataplane sinks at the captured
	// cgo bridge so TS 24.539 gate parameters (hold & forward,
	// TS 23.501 §5.27.4) and learned Ethernet MACs (TS 29.244
	// §8.2.98) land in the C session table. Must use the pre-swap
	// bridge — after SetBridge(router) the global is the PFCP router,
	// which has no dataplane gate.
	if tb, ok := dp.(upf.TSCGateBridge); ok {
		pfcp.SetTSCGateSink(func(imsi string, pduSessionID uint8, trafficClass uint8,
			baseTimeNS, cycleTimeNS uint64, gateStates []byte, durationsNS []uint32) {
			if err := tb.SetTSCGate(imsi, pduSessionID, upf.TSCGateConfig{
				TrafficClass: trafficClass,
				BaseTimeNS:   baseTimeNS,
				CycleTimeNS:  cycleTimeNS,
				GateStates:   gateStates,
				DurationsNS:  durationsNS,
			}); err != nil {
				log.Warnf("TSC gate → dataplane %s/%d: %v", imsi, pduSessionID, err)
			}
		})
		pfcp.SetTSCMACSink(func(imsi string, pduSessionID uint8, mac [6]byte) {
			if err := tb.LearnMAC(imsi, pduSessionID, mac); err != nil {
				log.Warnf("MAC learn → dataplane %s/%d: %v", imsi, pduSessionID, err)
			}
		})
		pfcp.SetTSCPTPSink(func(imsi string, pduSessionID uint8, enable bool) {
			if err := tb.SetPTPSteering(imsi, pduSessionID, enable); err != nil {
				log.Warnf("PTP steering → dataplane %s/%d: %v", imsi, pduSessionID, err)
			}
		})
	}

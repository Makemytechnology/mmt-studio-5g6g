...
// ── Rel-19 TSC (TS 23.501 §5.27.4) — see upf/tsc.go for the
//    Manager-level surface and dataplane/src/upf_dp_api.c for the
//    C implementation. ──

// SetTSCGate programs the IEEE 802.1Qbv egress gate of a session in
// the C dataplane (upf_dp_set_tsc_gate).
func (d *dpdkBridge) SetTSCGate(imsi string, pduSessionID uint8, cfg TSCGateConfig) error {
	if len(cfg.GateStates) == 0 || len(cfg.GateStates) != len(cfg.DurationsNS) {
		return fmt.Errorf("SetTSCGate: %d states / %d durations", len(cfg.GateStates), len(cfg.DurationsNS))
	}
	return dispatch(func() error {
		cIMSI := C.CString(imsi)
		defer C.free(unsafe.Pointer(cIMSI))
		states := make([]C.uint8_t, len(cfg.GateStates))
		durs := make([]C.uint32_t, len(cfg.DurationsNS))
		for i := range cfg.GateStates {
			states[i] = C.uint8_t(cfg.GateStates[i])
			durs[i] = C.uint32_t(cfg.DurationsNS[i])
		}
		rc := C.upf_dp_set_tsc_gate(cIMSI, C.uint8_t(pduSessionID),
			C.uint8_t(cfg.QFI), C.uint8_t(cfg.TrafficClass),
			C.uint64_t(cfg.BaseTimeNS), C.uint64_t(cfg.CycleTimeNS),
			&states[0], &durs[0], C.uint8_t(len(states)))
		if rc != 0 {
			return fmt.Errorf("upf_dp_set_tsc_gate rc=%d", int(rc))
		}
		return nil
	})
}

// ClearTSCGate deactivates the session gate.
func (d *dpdkBridge) ClearTSCGate(imsi string, pduSessionID uint8) error {
	return dispatch(func() error {
		cIMSI := C.CString(imsi)
		defer C.free(unsafe.Pointer(cIMSI))
		if C.upf_dp_clear_tsc_gate(cIMSI, C.uint8_t(pduSessionID)) != 0 {
			return fmt.Errorf("upf_dp_clear_tsc_gate failed")
		}
		return nil
	})
}

// LearnMAC records a UE MAC on an Ethernet PDU session (TS 29.244
// §8.2.98 MAC Addresses Detected source table).
func (d *dpdkBridge) LearnMAC(imsi string, pduSessionID uint8, mac [6]byte) error {
	return dispatch(func() error {
		cIMSI := C.CString(imsi)
		defer C.free(unsafe.Pointer(cIMSI))
		cmac := (*C.uint8_t)(unsafe.Pointer(&mac[0]))
		if rc := C.upf_dp_learn_mac(cIMSI, C.uint8_t(pduSessionID), cmac); rc != 0 {
			return fmt.Errorf("upf_dp_learn_mac rc=%d", int(rc))
		}
		return nil
	})
}

// SetPTPSteering toggles the fast-path (g)PTP processing of a session
// (TS 23.501 §5.27.1.2.2; dataplane upf_ptp.c).
func (d *dpdkBridge) SetPTPSteering(imsi string, pduSessionID uint8, enable bool) error {
	return dispatch(func() error {
		cIMSI := C.CString(imsi)
		defer C.free(unsafe.Pointer(cIMSI))
		en := C.uint8_t(0)
		...
		if C.upf_dp_set_ptp_steering(cIMSI, C.uint8_t(pduSessionID), en) != 0 {
			return fmt.Errorf("upf_dp_set_ptp_steering failed")
		}
		return nil
	})
}

...
type Handler struct {
	...
	// nwtt — network-side TSN translator of this UPF anchor (Rel-19
	// 5GS bridge, TS 23.501 §5.28.1: one bridge per UPF; see nwtt.go).
	nwtt *NWTT

	// Rel-19 clock drift reporting (TS 29.244 §7.4.4.1/§7.4.5.1):
	// armed by the CP's Clock Drift Control Information at
	// Association Setup; assocPeer is the CP endpoint Node Reports
	// target. Guarded by mu.
	clockDriftArmed bool
	assocPeer       *net.UDPAddr
}
...
func NewHandler(t *Transport, mgr ManagerHook) *Handler {
	h := &Handler{
		...
		mgr:      mgr,
		// One 5GS bridge per UPF anchor (TS 23.501 §5.28.1); the node
		// ID is made unique across anchors of this process so the TSN
		// AF sees distinct bridges per slice UPF.
		nwtt: NewNWTT(0x0A0B0C000000 | uint64(nwttInstanceSeq.Add(1))),
	}
	registerNWTT(h.nwtt)
...
// nwttInstanceSeq makes per-anchor 5GS bridge IDs unique in-process.
var nwttInstanceSeq atomic.Uint64

// NWTT exposes the anchor's network-side TSN translator (Rel-19).
func (h *Handler) NWTT() *NWTT { return h.nwtt }

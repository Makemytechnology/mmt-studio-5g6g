...
func (h *Handler) handleAssociationSetup(hdr *runtime.Header, payload []byte, peer *net.UDPAddr) {
	...
	// Rel-19 TSC — §7.4.4.1 Clock Drift Control Information: the CP
	// requests time-offset / cumulative-rateRatio reporting for
	// external (g)PTP time domains. Arm the §7.4.5.1 Node Report
	// path (see handler_node_report.go ReportClockDrift).
	if len(req.ClockDriftControlInformation) > 0 {
		flags := byte(0)
		if v := req.ClockDriftControlInformation[0].RequestedClockDriftInformation.Value; len(v) >= 1 {
			flags = v[0]
		}
		h.mu.Lock()
		h.clockDriftArmed = flags&0x03 != 0
		h.assocPeer = peer
		h.mu.Unlock()
		h.log.Infof("Clock drift reporting armed (RRTO=%d RRCR=%d) toward %s (§8.2.145)",
			flags&0x01, flags>>1&0x01, peer)
	}

...
func (h *Handler) handleSessionModification(hdr *runtime.Header, payload []byte, peer *net.UDPAddr) {
	...
	// Rel-19 Ethernet PDU sessions — Create PDRs arriving via §7.5.4
	// Modification can carry PDI Ethernet Packet Filters (TSC streams
	// installed by policymod after establishment). Register the
	// destination MACs with the NW-TT (mirrors the establishment
	// path) and report them to the CP (§8.2.98).
	if imsi != "" {
		var newMACs [][6]byte
		for i := range req.CreatePDR {
		...
		if len(newMACs) > 0 {
			h.reportDetectedMACs(imsi, pduSessionID, newMACs)
		}
	}

	// Rel-19 TSC — §7.5.4.18 TSC Management Information (IE 199):
	// terminate PMIC (NW-TT port management, TS 24.539 §6.2) and UMIC
	// (user plane node management, §6.3) at this anchor's NW-TT and
	// answer via §7.5.5.3 (IE 200) in the same response.
	for i := range req.TSCManagementInformation {
		tmi := &req.TSCManagementInformation[i]
		var reply genpfcp.TSCManagementInformationSMResp
		replied := false
		if tmi.PortManagementInformationContainer != nil {
			var port uint16
			if tmi.NWTTPortNumber != nil && len(tmi.NWTTPortNumber.Value) >= 4 {
				port = uint16(tmi.NWTTPortNumber.Value[2])<<8 | uint16(tmi.NWTTPortNumber.Value[3])
			}
			if out := h.nwtt.HandlePMIC(port, tmi.PortManagementInformationContainer.Value); out != nil {
				reply.PortManagementInformationContainer = &genpfcp.PortManagementInformationContainer{Value: out}
				if tmi.NWTTPortNumber != nil {
					reply.NWTTPortNumber = tmi.NWTTPortNumber
				}
				replied = true
			}
		}
		if tmi.UserPlaneNodeManagementInformationContainer != nil {
			if out := h.nwtt.HandleUMIC(tmi.UserPlaneNodeManagementInformationContainer.Value); out != nil {
				reply.UserPlaneNodeManagementInformationContainer = &genpfcp.UserPlaneNodeManagementInformationContainer{Value: out}
				replied = true
			}
		}
		if replied {
			resp.TSCManagementInformation = append(resp.TSCManagementInformation, reply)
		}
	}

...
func (h *Handler) handleSessionEstablishment(hdr *runtime.Header, payload []byte, peer *net.UDPAddr) {
	...
	if h.mgr != nil {
		...
		// Rel-19 Ethernet PDU sessions — §7.5.2.2 PDI Ethernet IEs:
		// Ethernet PDU Session Information (IE 142, ETHI flag = "all
		// DL traffic of the session") and Ethernet Packet Filter
		// (IE 132: MAC / Ethertype / C-TAG / S-TAG match keys). The
		// signalled MACs seed the NW-TT's per-port MAC table (§8.2.98
		// MAC Addresses Detected reporting reads from it).
		for i := range req.CreatePDR {
			pdi := &req.CreatePDR[i].PDI
			if pdi.EthernetPDUSessionInformation != nil &&
				len(pdi.EthernetPDUSessionInformation.Value) >= 1 &&
				pdi.EthernetPDUSessionInformation.Value[0]&0x01 != 0 {
				slog.Infof("PDR %d: Ethernet PDU Session Information ETHI=1 (§8.2.88 — session-level DL match)",
					i+1)
			}
			for j := range pdi.EthernetPacketFilter {
				for _, mac := range pdi.EthernetPacketFilter[j].MACAddress {
					// §8.2.93 MAC address IE: flags octet then the
					// flagged addresses; SOUR(bit1)/DEST(bit2).
					v := mac.Value
					if len(v) >= 7 && v[0]&0x03 != 0 {
						var m [6]byte
						copy(m[:], v[1:7])
						h.nwtt.RegisterDetectedMAC(imsi, pduSessionID, m)
					}
				}
			}
		}
	...
	// Rel-19 TSC — §7.5.2.1 Create Bridge Info for TSC (IE 194): when
	// the BII flag is set, allocate the DS-TT port for this Ethernet
	// PDU session and answer with §7.5.3.6 Created Bridge Info (IE
	// 195: Port Number + NW-TT Port Number(s) + 5GS User Plane Node
	// ID) — TS 23.502 §4.3.2.2.1 step 10b.
	if req.CreateBridgeInfoForTSC != nil &&
		len(req.CreateBridgeInfoForTSC.Value) >= 1 &&
		req.CreateBridgeInfoForTSC.Value[0]&0x01 != 0 && imsi != "" {
		dstt, nwtts, nodeID := h.nwtt.AllocateDSTTPort(imsi, pduSessionID)
		created := &genpfcp.CreatedBridgeInfoForTSC{
			DSTTPortNumber:      &genpfcp.DSTTPortNumber{Value: encodeCreatedBridgePort(dstt)},
			FiveGSUserPlaneNode: &genpfcp.FiveGSUserPlaneNode{Value: encodeNodeIDValue(nodeID)},
		}
		for _, p := range nwtts {
			created.NWTTPortNumber = append(created.NWTTPortNumber,
				genpfcp.NWTTPortNumber{Value: encodeCreatedBridgePort(p)})
		}
		resp.CreatedBridgeInfoForTSC = created
		slog.Infof("Created Bridge Info for TSC: node=%#x dsttPort=%d nwttPorts=%v (TS 29.244 §7.5.3.6)",
			nodeID, dstt, nwtts)
	}

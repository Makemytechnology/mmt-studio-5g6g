...
typedef struct {
    ...
    uint8_t    tsc_hold;       /* 1 if the Rel-19 TSC gate (TS 23.501 §5.27.4)
                                * is closed for this flow right now:
                                * DL → hold in dl_buf, UL → drop */
} upf_classify_result_t;

/* Evaluate the IEEE 802.1Qbv schedule of a TSC gate at `now_ns`
 * (5GS clock, epoch ns). Returns 1 when the gate is open for the
 * gate's traffic class, 0 when closed. Inactive/degenerate schedules
 * are treated as open (fail-open — protective mode is a CNC call). */

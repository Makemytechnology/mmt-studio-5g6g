...
/* ── TSC gate — Rel-19 hold & forward (TS 23.501 §5.27.4) ──
 * IEEE 802.1Qbv scheduled-traffic gate programmed by the TSN AF via
 * TS 24.539 set-parameter PMICs (GateEnabled / AdminBaseTime /
 * AdminControlList / AdminCycleTime) and pushed down through
 * upf_dp_set_tsc_gate(). The schedule is a cyclic list of
 * (gate-states bitmap, duration); a flow passes when the bit of its
 * traffic class is set in the entry covering "now" within the cycle.
 * DL packets hitting a closed gate are HELD in the session dl_buf and
 * flushed once the gate reopens (hold & forward); UL packets are
 * dropped and counted (the UL hold buffer lives in the DS-TT). */
#define UPF_TSC_GATE_MAX_ENTRIES 16
typedef struct __attribute__((__packed__)) {
    uint64_t base_time_ns;       /* AdminBaseTime, epoch ns */
    uint64_t cycle_time_ns;      /* AdminCycleTime */
    uint32_t duration_ns[UPF_TSC_GATE_MAX_ENTRIES];
    uint8_t  gate_states[UPF_TSC_GATE_MAX_ENTRIES]; /* bit N = TC N open */
    uint8_t  n_entries;
    uint8_t  qfi;                /* 0 = applies to every flow of the session */
    uint8_t  traffic_class;      /* bit checked in gate_states */
    uint8_t  _pad0;
    uint64_t held_pkts;          /* DL packets held by a closed gate */
    uint64_t gate_drops;         /* UL packets dropped by a closed gate */
    bool     active;
    uint8_t  _pad1[7];
} upf_tsc_gate_t;

/* ── Learned UE MACs (Ethernet PDU sessions, TS 29.244 §8.2.98) ── */
#define UPF_MAX_UE_MACS 8
...
typedef struct __attribute__((__packed__)) {
     ...
     * and while the Rel-19 TSC gate is closed (hold & forward). */
    upf_dl_buf_t *dl_buf;       /* heap-allocated, NULL if not buffering */

    /* Rel-19 TSC gate (TS 23.501 §5.27.4) — see upf_tsc_gate_t. */
    upf_tsc_gate_t tsc_gate;

    /* Learned UE MAC addresses (Ethernet PDU sessions; reported via
     * TS 29.244 §8.2.98 MAC Addresses Detected). */
    uint8_t  ue_mac[UPF_MAX_UE_MACS][6];
    uint8_t  n_ue_mac;
    /* ptp_steering — Rel-19 (g)PTP fast-path processing enabled
     * (TS 23.501 §5.27.1.2.2 TSi suffix / residence correction for
     * PTP-over-UDP; see upf_ptp.h). Set via upf_dp_set_ptp_steering
     * when a PTP instance is provisioned on the NW-TT. */
    uint8_t  ptp_steering;

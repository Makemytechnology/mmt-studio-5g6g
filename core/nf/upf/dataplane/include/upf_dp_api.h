...
/* ── Rel-19 TSC (TS 23.501 §5.27.4 hold & forward) ──
 * Program the IEEE 802.1Qbv egress gate of a PDU session: cyclic
 * schedule of (gate-states bitmap, duration_ns) starting at
 * base_time_ns with period cycle_time_ns. traffic_class selects the
 * bit checked in each gate_states octet; qfi scopes the gate to one
 * QoS flow (0 = whole session). n_entries ≤ UPF_TSC_GATE_MAX_ENTRIES.
 * DL packets in a closed window are held in the session DL buffer and
 * flushed when the gate reopens; UL packets are dropped (the UL hold
 * buffer is DS-TT-side). */
...
/* Learned-MAC table of an Ethernet PDU session (TS 29.244 §8.2.98
 * MAC Addresses Detected). learn is idempotent; get copies up to
 * max entries of 6 octets each into out and returns the count. */
int upf_dp_learn_mac(const char *imsi, uint8_t pdu_session_id,
                     const uint8_t mac[6]);

/* Enable/disable (g)PTP fast-path steering for a session (TS 23.501
 * §5.27.1.2.2 — TSi suffix at DL ingress, residence-time correction +
 * suffix strip at UL egress for PTP-over-UDP event messages). */

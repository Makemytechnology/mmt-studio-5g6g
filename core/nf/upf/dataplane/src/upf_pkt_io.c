...
static void process_ul_packet(uint8_t *buf, int len, struct sockaddr_in *from)
{
    ...
    /* Rel-19 TSC gate closed (TS 23.501 §5.27.4): the UL hold buffer
     * lives in the DS-TT — at the UPF a closed UL window means drop
     * (counted per-gate in the classifier). */
    if (result.tsc_hold) {
        __atomic_add_fetch(&io_stats.ul_dropped, 1, __ATOMIC_RELAXED);
        return;
    }
    ...
    if (action == 1 /* forward */) {
        ...
        /* Rel-19 (g)PTP steering — UL packets LEAVE the 5GS toward N6
         * here, so the NW-TT egress processing applies (TS 23.501
         * §5.27.1.2.2): fold the residence time since the ingress TSi
         * into correctionField and strip the 5GS suffix. Runs after
         * the UE-UE hairpin above (hairpinned copies stay inside the
         * 5GS and keep their suffix for the far DS-TT). */
        if (sess->ptp_steering) {
            struct timespec pts;
            clock_gettime(CLOCK_REALTIME, &pts);
            uint64_t pnow = (uint64_t)pts.tv_sec * 1000000000ULL + (uint64_t)pts.tv_nsec;
            decoded.payload_len = upf_ptp_process((uint8_t *)decoded.payload,
                                                  decoded.payload_len, decoded.payload_len,
                                                  0 /* egress */, pnow, NULL);
        }
...
static void process_dl_packet(uint8_t *ip_pkt, int ip_len)
{
    ...
    /* Rel-19 TSC hold & forward (TS 23.501 §5.27.4): the DL egress
     * gate is closed for this flow's window — HOLD the packet in the
     * session dl_buf instead of forwarding. Reuses the §5.2.1 buffer
     * ring; flushed below once the gate reopens. No §7.5.8.2 DLDR is
     * raised: the UE is CM-CONNECTED with an active tunnel — this is
     * scheduling, not paging. */
    if (result.tsc_hold && sess && action == 1) {
        if (!sess->dl_buf)
        ...
        if (sess->dl_buf && sess->dl_buf->count < UPF_DL_BUF_SLOTS) {
            ...
            sess->dl_buf->count++;
            __atomic_add_fetch(&sess->tsc_gate.held_pkts, 1, __ATOMIC_RELAXED);
            RTE_LOG(DEBUG, UPF, "TSC gate closed: held DL pkt %u/%u for %s/%u (§5.27.4)\n",
                    sess->dl_buf->count, UPF_DL_BUF_SLOTS,
                    sess->imsi, sess->pdu_session_id);
        } else {
            /* Hold buffer exhausted — drop (protective). */
            __atomic_add_fetch(&io_stats.dl_dropped, 1, __ATOMIC_RELAXED);
        }
        return;
    }

    /* TSC gate open again: flush any held packets ahead of the fresh
     * one so ordering within the flow is preserved (hold & FORWARD). */
    if (!result.tsc_hold && sess && sess->tsc_gate.active &&
        sess->dl_buf && sess->dl_buf->count > 0 &&
        action == 1 && result.far) {
        int flushed = upf_pkt_io_flush_dl_buf(sess, result.far);
        if (flushed > 0)
            RTE_LOG(INFO, UPF, "TSC gate reopened: flushed %d held DL pkt(s) for %s/%u (§5.27.4)\n",
    ...
    if (action == 1 /* forward */ && result.far && result.far->ohc_type == 1) {
        /* Rel-19 (g)PTP steering — DL packets ENTER the 5GS here, so
         * the NW-TT ingress processing applies (TS 23.501
         * §5.27.1.2.2): stamp TSi (5G internal system clock) into the
         * suffix of PTP-over-UDP messages. The egress correction
         * happens at the DS-TT (or the UL path below on hairpins). */
        if (sess && sess->ptp_steering) {
            struct timespec pts;
            clock_gettime(CLOCK_REALTIME, &pts);
            uint64_t pnow = (uint64_t)pts.tv_sec * 1000000000ULL + (uint64_t)pts.tv_nsec;
            ip_len = upf_ptp_process(ip_pkt, (uint16_t)ip_len, PKT_BUF_SZ,
                                     1 /* ingress */, pnow, NULL);
        }

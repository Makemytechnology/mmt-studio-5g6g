...
/* ═══════════════════════════════════════════════════════════════
 * Rel-19 TSC — TS 23.501 §5.27.4 hold & forward gate + Ethernet
 * MAC learning (TS 29.244 §8.2.98). See upf_types.h upf_tsc_gate_t
 * and upf_classifier.c upf_tsc_gate_is_open() for the enforcement.
 * ═══════════════════════════════════════════════════════════════ */

int upf_dp_set_tsc_gate(const char *imsi, uint8_t pdu_session_id,
                        uint8_t qfi, uint8_t traffic_class,
                        uint64_t base_time_ns, uint64_t cycle_time_ns,
                        const uint8_t *gate_states,
                        const uint32_t *durations_ns, uint8_t n_entries)
{
    upf_session_t *sess = upf_session_get(imsi, pdu_session_id);
    if (!sess) return -1;
    if (n_entries == 0 || n_entries > UPF_TSC_GATE_MAX_ENTRIES ||
        !gate_states || !durations_ns || cycle_time_ns == 0)
        return -2;

    upf_tsc_gate_t *g = &sess->tsc_gate;
    g->active = false;  /* quiesce fast path while rewriting */
    __atomic_thread_fence(__ATOMIC_RELEASE);
    g->base_time_ns  = base_time_ns;
    g->cycle_time_ns = cycle_time_ns;
    g->qfi           = qfi;
    g->traffic_class = traffic_class & 7;
    g->n_entries     = n_entries;
    for (uint8_t i = 0; i < n_entries; i++) {
        g->gate_states[i] = gate_states[i];
        g->duration_ns[i] = durations_ns[i];
    }
    __atomic_thread_fence(__ATOMIC_RELEASE);
    g->active = true;

    RTE_LOG(INFO, UPF,
            "TSC gate set for %s/%u: qfi=%u tc=%u cycle=%luns entries=%u (TS 23.501 §5.27.4)\n",
            imsi, pdu_session_id, qfi, g->traffic_class,
            (unsigned long)cycle_time_ns, n_entries);
    return 0;
}

int upf_dp_clear_tsc_gate(const char *imsi, uint8_t pdu_session_id)
{
    upf_session_t *sess = upf_session_get(imsi, pdu_session_id);
    if (!sess) return -1;
    sess->tsc_gate.active = false;
    RTE_LOG(INFO, UPF, "TSC gate cleared for %s/%u\n", imsi, pdu_session_id);
    return 0;
}

int upf_dp_learn_mac(const char *imsi, uint8_t pdu_session_id,
                     const uint8_t mac[6])
{
    upf_session_t *sess = upf_session_get(imsi, pdu_session_id);
    if (!sess || !mac) return -1;
    for (uint8_t i = 0; i < sess->n_ue_mac; i++) {
        if (memcmp(sess->ue_mac[i], mac, 6) == 0)
            return 0;  /* already learned */
    }
    if (sess->n_ue_mac >= UPF_MAX_UE_MACS)
        return -2;
    memcpy(sess->ue_mac[sess->n_ue_mac], mac, 6);
    sess->n_ue_mac++;
    RTE_LOG(INFO, UPF,
            "MAC learned for %s/%u: %02x:%02x:%02x:%02x:%02x:%02x (%u total, §8.2.98)\n",
            imsi, pdu_session_id, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5],
            sess->n_ue_mac);
    return 0;
}

int upf_dp_set_ptp_steering(const char *imsi, uint8_t pdu_session_id,
                            uint8_t enable)
{
    upf_session_t *sess = upf_session_get(imsi, pdu_session_id);
    if (!sess) return -1;
    sess->ptp_steering = enable ? 1 : 0;
    RTE_LOG(INFO, UPF, "PTP steering %s for %s/%u (TS 23.501 §5.27.1.2.2)\n",
            enable ? "enabled" : "disabled", imsi, pdu_session_id);
    return 0;
}

int upf_dp_get_macs(const char *imsi, uint8_t pdu_session_id,
                    uint8_t *out, uint8_t max)
{
    upf_session_t *sess = upf_session_get(imsi, pdu_session_id);
    if (!sess || !out) return -1;
    uint8_t n = sess->n_ue_mac;
    if (n > max) n = max;
    for (uint8_t i = 0; i < n; i++)
        memcpy(out + i * 6, sess->ue_mac[i], 6);
    return n;
}

...
/* upf_tsc_gate_is_open — walk the IEEE 802.1Qbv AdminControlList and
 * report whether the entry covering `now` opens the gate for the
 * flow's traffic class (Rel-19 TSC, TS 23.501 §5.27.4). The schedule
 * repeats every cycle_time_ns from base_time_ns; per IEEE 802.1Q
 * §8.6.9 the cycle extends both forward and backward from the base
 * time, so times before AdminBaseTime map into the cycle as well. */
int upf_tsc_gate_is_open(const upf_tsc_gate_t *g, uint64_t now_ns)
{
    if (!g->active || g->cycle_time_ns == 0 || g->n_entries == 0)
        return 1;
    uint64_t phase;
    if (now_ns >= g->base_time_ns) {
        phase = (now_ns - g->base_time_ns) % g->cycle_time_ns;
    } else {
        uint64_t back = (g->base_time_ns - now_ns) % g->cycle_time_ns;
        phase = back ? g->cycle_time_ns - back : 0;
    }
    uint8_t n = g->n_entries;
    if (n > UPF_TSC_GATE_MAX_ENTRIES) n = UPF_TSC_GATE_MAX_ENTRIES;
    uint64_t acc = 0;
    for (uint8_t i = 0; i < n; i++) {
        acc += g->duration_ns[i];
        if (phase < acc)
            return (g->gate_states[i] >> (g->traffic_class & 7)) & 1;
    }
    /* Phase beyond the programmed windows (list shorter than the
     * cycle): IEEE 802.1Q keeps the last gate state. */
    return (g->gate_states[n - 1] >> (g->traffic_class & 7)) & 1;
}
                        ...
                        upf_classify_result_t *result)
{
    ...
    /* Step 2a½: Rel-19 TSC gate (TS 23.501 §5.27.4 hold & forward —
     * IEEE 802.1Qbv schedule programmed by the TSN AF). The flow's
     * QFI comes from the GTP-U header (UL) or the matched PDR (DL).
     * A closed gate marks tsc_hold — the pkt_io layer HOLDs DL
     * packets in the dl_buf and drops UL (the UL hold buffer lives
     * in the DS-TT, not here). */
    if (sess->tsc_gate.active) {
        uint8_t flow_qfi = qfi;
        if (flow_qfi == 0 && result->matched_pdr)
            flow_qfi = result->matched_pdr->qfi;
        if (sess->tsc_gate.qfi == 0 || sess->tsc_gate.qfi == flow_qfi) {
            struct timespec ts;
            clock_gettime(CLOCK_REALTIME, &ts);
            uint64_t now = (uint64_t)ts.tv_sec * 1000000000ULL + (uint64_t)ts.tv_nsec;
            if (!upf_tsc_gate_is_open(&sess->tsc_gate, now)) {
                result->tsc_hold = 1;
                if (direction == 0)
                    __atomic_add_fetch(&sess->tsc_gate.gate_drops, 1, __ATOMIC_RELAXED);
            }
        }
    }

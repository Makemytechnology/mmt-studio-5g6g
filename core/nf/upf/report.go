...
const (
	...
	// Go-side only (never produced by the C ring; the SMF-side PFCP
	// bridge synthesises them from inbound wire messages):
	ReportClockDrift ReportType = 6 // §7.4.5.1 Node Report → Clock Drift Report
	ReportEthMAC     ReportType = 7 // §7.5.8.3 Usage Report → Ethernet Traffic Information
)
...
// ClockDriftPayload — TS 29.244 §7.4.5.1 Clock Drift Report contents
// (IEs 206/209/210): offset between the 5GS clock and an external
// (g)PTP time domain.
type ClockDriftPayload struct {
	TimeDomain   uint8
	TimeOffsetNS int64
	CumRateRatio int32
	HasRateRatio bool
...
// EthMACPayload — TS 29.244 §8.2.98 MAC Addresses Detected carried in
// a §7.5.8.3 Usage Report (Ethernet PDU sessions).
type EthMACPayload struct {
	MACs [][6]byte
}
...
type Report struct {
	...
	ClockDrift *ClockDriftPayload
	EthMAC     *EthMACPayload
}

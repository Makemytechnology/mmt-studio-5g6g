...
var TsnDDL = []string{
	...
	// Default Ethernet DNN for TSC — an Ethernet PDU session (TS 23.501
	// §5.28.1) carries no UE IP, so no apn_ip_pools row. Same pattern
	// as the 'v2x' DNN in v2x.go: the feature schema ships its DNN so
	// TSN works out of the box; operators tune it from the GUI.
	`INSERT OR IGNORE INTO apn_config
	   (apn_name, ambr_dl_kbps, ambr_ul_kbps, pdu_session_type, ssc_mode, mtu)
	 VALUES ('tsn', 100000, 100000, 'Ethernet', 1, 1500)`,
}

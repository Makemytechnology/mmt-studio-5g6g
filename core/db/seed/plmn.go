...
func SeedPLMN(db *sql.DB) error {
	...
	// Must cover every S-NSSAI the seeded subscribers hold (see
	// SeedDefaultUEs / nssai_catalog): the Allowed NSSAI is the
	// intersection of the Subscribed S-NSSAIs with the slices the
	// serving PLMN supports (TS 23.501 §5.15.4) — a subscriber slice
	// missing here can never register (reject #62, TS 24.501
	// §5.5.1.2.5). The URLLC and mIoT UE blocks subscribe to
	// (2:000002) / (3:000003).
	for _, s := range []struct{ sst int; sd string }{
		{1, "000001"}, // eMBB
		{2, "000001"}, // URLLC
		{2, "000002"}, // URLLC (subscriber block)
		{3, "000001"}, // mIoT
		{3, "000003"}, // mIoT (subscriber block)
	} {

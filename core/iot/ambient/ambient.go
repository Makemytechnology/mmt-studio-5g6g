...
func RegisterTag(tagID, tagClass, tagType string, groupID, owner *string) error {
	...
	// Upsert: re-registration refreshes the record — an operator
	// re-provisioning an existing tag id is a config update, not an
	// error (mirrors nbiot.SetPSM's ON CONFLICT semantics).
	_, err := engine.Exec(`INSERT INTO iot_tags
		(tag_id, tag_class, tag_type, group_id, owner)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(tag_id) DO UPDATE SET
		  tag_class=excluded.tag_class, tag_type=excluded.tag_type,
		  group_id=excluded.group_id, owner=excluded.owner`,
...
func RegisterReader(readerID string, gnbIP, capabilities *string,
	...
	_, err := engine.Exec(`INSERT INTO iot_readers
		...
		ON CONFLICT(reader_id) DO UPDATE SET
		  gnb_ip=excluded.gnb_ip, latitude=excluded.latitude,
		  longitude=excluded.longitude, capabilities=excluded.capabilities,
		  status='active'`,

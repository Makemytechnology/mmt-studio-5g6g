...
# Lazily-populated cache for Get-OrchDir's resolved orchestrate path.
# MUST be declared here (value $null), not merely assigned inside
# Get-OrchDir: the function's first line reads it (if ($script:_OrchDir))
# as a cache probe, and under Set-StrictMode -Version Latest reading a
# never-assigned variable is a terminating error -- which is what
# produced the red "variable '$script:_OrchDir' cannot be retrieved
# because it has not been set" on every up/down/status. Seeding it to
# $null makes that first probe safe (falsy) until Get-OrchDir fills it.
$script:_OrchDir = $null

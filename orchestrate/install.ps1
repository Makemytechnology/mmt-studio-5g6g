...
# Windows PowerShell 5.1 renders a per-byte progress bar for
# Invoke-WebRequest by default, and a long-standing engine bug makes
# that throttle downloads by 10-50x (a browser-speed CDN drops to a
# crawl). Silencing progress is THE fix that makes script downloads as
# fast as downloading in a browser. Harmless in PowerShell 7+.
...
function Wait-DockerEngineReady {
    ...
    # reachable via Docker Desktop's WSL integration). Returns elapsed
    # seconds on success, -1 on timeout, -2 when WSL integration is
    # confirmed OFF for the distro. Heartbeats every 30 s.
    #
    # IMPORTANT: probe in a LOGIN shell (`bash -lc`), exactly how
    # install.sh is launched later. A non-login `bash -c` inherits the
    # Windows PATH via WSL interop, so `docker` there resolves to the
    # Windows docker.exe and reports success even when the Linux-native
    # WSL integration is disabled — a false positive that let the run
    # sail past this gate and then fail cryptically inside install.sh
    # with "The command 'docker' could not be found in this WSL 2
    # distro." A login shell resets PATH to the Linux default (dropping
    # the Windows PATH), so it tests the exact docker install.sh needs.
    param(
    ...
    $start    = Get-Date
    $lastBeat = 0
    # OK  = native docker works. NOINTEG = Docker Desktop's
    # integration-disabled stub answered (integration off, not a
    # timing issue). NO = anything else (engine still starting).
    #
    # Pass the probe to bash as base64, NOT as an inline -lc string.
    # The script mixes spaces, single quotes and a double-quoted grep
    # pattern; handing that straight to `wsl ... -- bash -lc <string>`
    # makes PowerShell + wsl.exe + bash each re-parse the quotes and
    # bash dies with "syntax error: unexpected end of file". Base64 has
    # no shell metacharacters, so it survives every layer intact; we
    # decode and run it in a login shell (`bash -l`) so PATH matches how
    # install.sh runs.
    $probeScript = @'
docker version --format '{{.Server.Version}}' >/dev/null 2>/tmp/mmt_dv.err
rc=$?
if [ $rc -eq 0 ]; then echo OK
elif grep -qi "could not be found in this WSL" /tmp/mmt_dv.err; then echo NOINTEG
else echo NO
fi
'@
    ...
    while ((Get-Date) -lt $deadline) {
        $probeRaw = (wsl -d $Distro -u root -- bash -lc "echo $probeB64 | base64 -d | bash -l" 2>$null)
        $probe = @($probeRaw) | Where-Object { $_ -match '^(OK|NOINTEG|NO)$' } | Select-Object -Last 1
        if ($probe -eq 'OK') {
            return [int]((Get-Date) - $start).TotalSeconds
        }
        if ($probe -eq 'NOINTEG') {
            # Give Docker Desktop a grace window to mount integration
            # during a fresh start before declaring it off, so a normal
            # cold boot isn't misreported.
            if ($null -eq $noIntegSince) { $noIntegSince = Get-Date }
            elseif (((Get-Date) - $noIntegSince).TotalSeconds -ge 75) {
                return -2
            }
        } else {
            $noIntegSince = $null
...
if (-not $SkipDockerDesktop) {
    ...
    if (-not (Test-Path $ddExe)) {
        ...
        # Unique per-run filename. A fixed 'DockerDesktopInstaller.exe'
        # left over from a previous run can still be held by antivirus, a
        # hung download, or a running installer, which makes
        # Invoke-WebRequest fail with "The process cannot access the file
        # ... because it is being used by another process." A GUID name
        # sidesteps any stale lock, and we retry a few times for good
        # measure before pointing the operator at a manual install.
        $dl = Join-Path $env:TEMP ("DockerDesktopInstaller-{0}.exe" -f ([guid]::NewGuid().ToString('N')))
        $downloaded = $false
        for ($attempt = 1; $attempt -le 3; $attempt++) {
            try {
                # Fast path: curl.exe. It ships in Windows 10/11
                # (System32\curl.exe), downloads at full CDN speed, and is
                # immune to PowerShell 5.1's progress-bar throttling that
                # cripples Invoke-WebRequest. -L follows the CDN redirect,
                # --fail turns an HTTP error page into a non-zero exit (so
                # the retry loop catches it), and --retry adds transport
                # resilience. Fall back to BITS, then Invoke-WebRequest
                # (progress silenced globally, which is the real speed fix),
                # if curl.exe is somehow absent.
                $curl = Join-Path $env:SystemRoot 'System32\curl.exe'
                if (-not (Test-Path $curl)) { $curl = 'curl.exe' }
                $downloadedBy = $null
                try {
                    & $curl -L --fail --retry 3 --retry-delay 2 -o $dl $url
                    if ($LASTEXITCODE -ne 0) { throw "curl.exe exited $LASTEXITCODE" }
                    $downloadedBy = 'curl.exe'
                } catch {
                    Write-Note "curl.exe download failed ($($_.Exception.Message)); trying BITS/IWR"
                    Remove-Item $dl -Force -ErrorAction SilentlyContinue
                    try {
                        Import-Module BitsTransfer -ErrorAction Stop
                        Start-BitsTransfer -Source $url -Destination $dl -ErrorAction Stop
                        $downloadedBy = 'BITS'
                    } catch {
                        Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile $dl
                        $downloadedBy = 'Invoke-WebRequest'
                    }
                }
                # Sanity-check the payload: the Docker installer is >100 MB,
                # so anything tiny is a captive-portal HTML page or a partial
                # write, not the real binary.
                if (-not (Test-Path $dl) -or ((Get-Item $dl).Length -lt 50MB)) {
                    throw "downloaded file missing or too small ($([math]::Round((Get-Item $dl -ErrorAction SilentlyContinue).Length/1MB,1)) MB)"
                }
                Write-Note ("downloaded {0} MB via {1}" -f [math]::Round((Get-Item $dl).Length/1MB,0), $downloadedBy)
                $downloaded = $true
                break
            } catch {
                Write-Note "download attempt $attempt/3 failed: $($_.Exception.Message)"
                Remove-Item $dl -Force -ErrorAction SilentlyContinue
                Start-Sleep -Seconds 3
            }
        }
        if (-not $downloaded) {
            throw ("Could not download the Docker Desktop installer after 3 attempts. " +
                "A previous copy may be locked by antivirus or a still-running installer. " +
                "Close any running 'Docker Desktop Installer', delete " +
                "$env:TEMP\DockerDesktopInstaller*.exe, then re-run this script — or install " +
                "Docker Desktop manually from https://www.docker.com/products/docker-desktop/ " +
                "and re-run (this step is skipped once Docker Desktop is installed).")
        }
    ...
    # Best-effort: also flip the "enable integration with my default WSL
    # distro" flag. Some Docker Desktop versions gate the per-distro list
    # behind this, and if $Distro is the default distro it's the only
    # switch that matters. Harmless when the key is ignored.
    $defProp = $cfg.PSObject.Properties['enableIntegrationWithDefaultWslDistro']
    if (-not ($defProp -and [bool]$defProp.Value)) {
        $cfg | Add-Member -NotePropertyName enableIntegrationWithDefaultWslDistro -NotePropertyValue $true -Force
        $changed = $true
    }
    if ($changed) {
    ...
    if ($waitSec -eq -2) {
        throw @"
Docker Desktop is running, but its WSL integration for '$Distro' is OFF,
so the Linux-native 'docker' command is missing inside the distro (that's
the "could not be found in this WSL 2 distro" message). The build needs it.

Fix (takes ~1 min):
  1. Open Docker Desktop.
  2. Settings (gear) -> Resources -> WSL Integration.
  3. Turn ON the toggle for '$Distro' (and 'Enable integration with my
     default WSL distro' if shown).
  4. Click 'Apply & restart'.
  5. Verify from a terminal:  wsl -d $Distro -- docker version
     (it should print Client AND Server versions, no stub message)
  6. Re-run this installer.
"@
    }
    if ($waitSec -lt 0) {
        throw "Docker engine not reachable from $Distro after 15 minutes. Open Docker Desktop, verify Settings -> Resources -> WSL Integration lists '$Distro' and 'Apply & restart', then re-run."
    }
...
# --inplace: write straight to the destination file instead of rsync's
# default "write a .<name>.XXXXXX temp then rename it into place" dance.
# On WSL that random-extension temp file (on the ext4 side) is exactly
# what antivirus / Windows Defender scanning the WSL2 filesystem tends to
# grab and delete mid-flight, which surfaced as:
#   rsync: rename ".04_stress.robot.XXXX" -> "...": No such file or
#   directory (2)   ...   rsync error: some files were not transferred
#   (code 23)
# --inplace removes the temp+rename step, so that failure mode can't
# occur. Staging is also retried (in PowerShell, to avoid fragile inline
# bash quoting) for any transient 9p glitch.
$stageCmd = "set -e; mkdir -p '$wslLinuxRoot'; rsync -a --inplace --info=name0,progress2 --delete --exclude='.git/' --exclude='release-tmp/' --exclude='dist/' '$wslSource/' '$wslLinuxRoot/'; find '$wslLinuxRoot' -type f -name '*.sh' -exec sed -i 's/\r`$//' {} +; chmod +x '$wslLinuxRoot/orchestrate/install.sh'"
$staged = $false
for ($attempt = 1; $attempt -le 3; $attempt++) {
    wsl -d $Distro -u root -- bash -lc "$stageCmd"
    if ($LASTEXITCODE -eq 0) { $staged = $true; break }
    Write-Note "source copy attempt $attempt/3 failed (rsync exit $LASTEXITCODE; often antivirus touching a WSL temp file) — retrying in 5s..."
    Start-Sleep -Seconds 5
}
if (-not $staged) {
    throw ("Copying the source into $Distro failed after 3 attempts (rsync). This is " +
        "usually antivirus / Windows Defender scanning the WSL2 filesystem and removing " +
        "rsync's temporary files. Add a Defender exclusion for the distro " +
        "(\\wsl.localhost\$Distro) or for the source folder, or briefly pause real-time " +
        "protection, then re-run.")
}

$argString = ($installArgs | ForEach-Object { "'$_'" }) -join ' '
$runCmd = "stdbuf -oL -eL bash '$wslLinuxRoot/orchestrate/install.sh' $argString"
wsl -d $Distro -u root -- bash -lc "$runCmd"
...
if (-not $NoWireshark) {
    Write-Host ""
    # NOTE: we deliberately DO NOT arm the Wireshark watcher from here.
    # install.ps1 runs ELEVATED, so any watcher it spawns is a
    # High-integrity process. run_studio.bat runs at normal (Medium)
    # integrity and cannot Stop-Process a High-integrity watcher, so its
    # own `up` would fail to replace this one and end up arming a SECOND
    # watcher — two watchers then each open their own Wireshark on the
    # next test (the "two Wireshark windows" bug). Leaving the watcher to
    # run_studio.bat gives it a single, Medium-integrity owner that can
    # always be stopped/replaced cleanly. The first `run_studio.bat`
    # (its default action arms the watcher) will start exactly one.
    if (-not (Test-Path $wsScript)) {
        Write-Note "Wireshark watcher will be available via run_studio.bat"
    } else {
        Write-Ok "Wireshark: run_studio.bat arms the per-test watcher (one window per test)"
    }
}

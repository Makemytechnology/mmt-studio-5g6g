...
# Belt-and-suspenders against arming the watcher twice (e.g. two
# `run_studio.bat up` runs, or two open terminals). A named mutex in the
# session-local namespace lets any second instance detect the first and
# bow out, so only ONE watcher polls /api/tests and only ONE Wireshark
# opens per test.
#
# NOTE: Local\ (not Global\) on purpose — creating a Global\ object needs
# SeCreateGlobalPrivilege, which a normal non-elevated user does not have,
# so a Global\ mutex would throw for the very (Medium-integrity) process
# that run_studio.bat launches. Local\ is shared across every process in
# the interactive session, which is all we need. The cross-elevation case
# (an *elevated* install-time watcher) is handled separately by no longer
# arming a watcher from the elevated installer — see install.ps1.
$script:SingletonMutex = $null
try {
    $mutexName = 'Local\MMT_PerTestWiresharkWatcher'
    $createdNew = $false
    $script:SingletonMutex = New-Object System.Threading.Mutex($true, $mutexName, [ref]$createdNew)
    $owned = $createdNew
    if (-not $createdNew) {
        # Mutex already exists — another watcher created it. Try to grab
        # it without blocking; if we can't, that watcher is alive.
        try { $owned = $script:SingletonMutex.WaitOne(0) }
        catch [System.Threading.AbandonedMutexException] { $owned = $true }
    }
    if (-not $owned) {
        Write-Note "another Wireshark watcher is already running -- exiting so only ONE Wireshark opens per test"
        exit 0
    }
    Write-Ok "single-instance lock acquired"
} catch {
    # Guard is best-effort: if the mutex APIs are unavailable for any
    # reason, fall through rather than block the watcher entirely.
    Write-Note "single-instance guard unavailable ($($_.Exception.Message)) -- continuing"
}
...
$cancelHandler = {
    ...
    Stop-LiveWireshark
    # Release the single-instance lock so the next watcher can arm.
